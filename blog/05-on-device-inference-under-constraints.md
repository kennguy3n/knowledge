...
> inference across an ordered adapter list (MLX → llama.cpp →
> Fallback) with device-tier gating, so the same app degrades
...
task to the first one available on the current device. The
[inference-routing spec](../docs/technical/inference-routing.md) is the
full reference; the default order is:
...
device tier)`: lower-tier devices degrade to a lighter task profile or
the fallback rather than attempting an inference that would thrash or
OOM. This is the mechanism that lets one binary span the budget-phone-
to-laptop range.
...
```text
// configure once: ordered adapters + device tier
// then request synthesis; the router picks the adapter
synthesize(scope_id, task) -> result   // MLX? llama.cpp? fallback?
```
