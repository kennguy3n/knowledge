...
#![deny(missing_docs)]
// N-API entry points take owned `String` / `Vec<u8>` by value
// because the JS bridge generated by `napi-derive` hands ownership
// across the language boundary — every JS-side argument arrives as
// a freshly-allocated `String`. Borrowed equivalents would force an
// extra copy on every call. Silenced at module level for the same
// reason as the sibling `ffi` crate.
