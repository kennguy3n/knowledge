//! Microsoft Teams connector — Graph channel messages.
//!
//! A Teams connector instance ingests one channel's message stream
//! (`/teams/{team-id}/channels/{channel-id}/messages`). Graph exposes a
//! `messages/delta` change feed shaped exactly like the OneDrive /
//! SharePoint drive delta, so the sync logic mirrors those connectors:
//!
//! * `authenticate` exchanges the authorization code via the wired
//!   [`OAuth2CodeExchange`].
//! * `initial_sync` walks `…/messages/delta` from scratch, follows
//!   `@odata.nextLink`, and surfaces the final `@odata.deltaLink` as the
//!   cursor; emits one [`ConnectorEvent`] per message.
//! * `incremental_sync` GETs the stored `@odata.deltaLink` verbatim.
//! * `fetch_content` GETs a single message and returns its HTML body
//!   flattened to text.
//! * `subscribe_webhook` POSTs `/subscriptions` for a Graph change
//!   notification on the channel messages resource (Teams caps message
//!   subscriptions at ~60 minutes; we sit under that).
//! * `handle_webhook_event` drains the entire
//!   `changeNotificationCollection` batch, skipping unknown
//!   `changeType`s.
//!
//! Wiring contract mirrors the other connectors: the constructor takes
//! an `Arc<dyn HttpTransport>` and an `Arc<dyn OAuth2CodeExchange>`.

use std::sync::Arc;

use chrono::{DateTime, Duration, Utc};
use connector_framework::{
    bearer_get_json, bearer_post_json, percent_encode_path_component, Connector, ConnectorConfig,
    ConnectorError, ConnectorEvent, ConnectorInstanceId, FetchedContent, HttpTransport,
    OAuth2CodeExchange, OAuth2Token, Result, SourceDocumentId, SyncRunResult, SyncState,
    WebhookEventTypes, WebhookSecret, WebhookSubscription,
};
use serde::{Deserialize, Serialize};

use crate::content::strip_html;

/// Default Graph base URL. Override via `auth_config_json.api_base_url`.
pub const DEFAULT_API_BASE_URL: &str = "https://graph.microsoft.com";

/// Default Graph API version path.
pub const DEFAULT_API_VERSION: &str = "/v1.0";

/// Default substrate-side subscription TTL — Teams channel-message
/// subscriptions cap at ~60 minutes; sit comfortably under.
pub const DEFAULT_SUBSCRIPTION_TTL_MINUTES: i64 = 55;

/// Safety ceiling on number of delta pages a single sync will walk.
pub const MAX_DELTA_PAGES: usize = 10_000;

/// The body facet of a Teams `chatMessage`.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MessageBody {
    /// `html` or `text`.
    #[serde(default, rename = "contentType")]
    pub content_type: String,
    /// Raw body content (HTML or plain text).
    #[serde(default)]
    pub content: String,
}

/// One Teams `chatMessage` (subset relevant to substrate ingestion).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ChatMessage {
    /// Message id.
    #[serde(default)]
    pub id: String,
    /// Optional subject line.
    #[serde(default)]
    pub subject: Option<String>,
    /// Creation time.
    #[serde(default, rename = "createdDateTime")]
    pub created_date_time: Option<DateTime<Utc>>,
    /// Last-modified time.
    #[serde(default, rename = "lastModifiedDateTime")]
    pub last_modified_date_time: Option<DateTime<Utc>>,
    /// Soft-delete marker — non-null when the message was deleted.
    #[serde(default, rename = "deletedDateTime")]
    pub deleted_date_time: Option<DateTime<Utc>>,
    /// Message body.
    #[serde(default)]
    pub body: Option<MessageBody>,
}

/// One page of `…/messages/delta` results.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MessageDeltaResponse {
    /// Messages that changed in this page.
    #[serde(default)]
    pub value: Vec<ChatMessage>,
    /// Forward link to the next page (mid-walk).
    #[serde(default, rename = "@odata.nextLink")]
    pub next_link: Option<String>,
    /// Final-state cursor — present on the last page only.
    #[serde(default, rename = "@odata.deltaLink")]
    pub delta_link: Option<String>,
}

/// `/subscriptions` create response.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct GraphSubscriptionResponse {
    /// Subscription id Graph assigned (needed for revoke).
    #[serde(default)]
    pub id: Option<String>,
    /// Echoed expiry (RFC-3339).
    #[serde(default, rename = "expirationDateTime")]
    pub expiration_date_time: Option<DateTime<Utc>>,
}

/// One `changeNotification` in a Graph subscription batch.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ChangeNotification {
    /// Substrate-side resource id.
    pub resource: String,
    /// Lifecycle string: `created`, `updated`, `deleted`.
    #[serde(rename = "changeType")]
    pub change_type: String,
    /// Subscription id Graph uses for delivery routing.
    #[serde(default, rename = "subscriptionId")]
    pub subscription_id: String,
    /// Wall-clock event time.
    #[serde(default, rename = "eventTime")]
    pub event_time: Option<DateTime<Utc>>,
}

/// `changeNotificationCollection` payload Graph POSTs to the
/// `notificationUrl`.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ChangeNotificationCollection {
    /// Notification batch.
    #[serde(default)]
    pub value: Vec<ChangeNotification>,
}

/// Microsoft Teams connector. Holds the wired transport + OAuth.
pub struct TeamsConnector {
    /// Connector instance id (used by `subscribe_webhook`).
    pub instance: ConnectorInstanceId,
    transport: Arc<dyn HttpTransport>,
    oauth: Arc<dyn OAuth2CodeExchange>,
    api_base_url: String,
    api_version: String,
}

impl std::fmt::Debug for TeamsConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TeamsConnector")
            .field("instance", &self.instance)
            .field("api_base_url", &self.api_base_url)
            .field("api_version", &self.api_version)
            .field("transport", &"<HttpTransport>")
            .field("oauth", &"<OAuth2CodeExchange>")
            .finish()
    }
}

impl TeamsConnector {
    /// Construct a Teams connector.
    pub fn new(
        instance: ConnectorInstanceId,
        transport: Arc<dyn HttpTransport>,
        oauth: Arc<dyn OAuth2CodeExchange>,
    ) -> Self {
        Self {
            instance,
            transport,
            oauth,
            api_base_url: DEFAULT_API_BASE_URL.to_string(),
            api_version: DEFAULT_API_VERSION.to_string(),
        }
    }

    /// Override the Graph REST base URL.
    #[must_use]
    pub fn with_api_base_url(mut self, url: impl Into<String>) -> Self {
        self.api_base_url = url.into();
        self
    }

    /// Override the Graph API version segment.
    #[must_use]
    pub fn with_api_version(mut self, version: impl Into<String>) -> Self {
        self.api_version = version.into();
        self
    }

    fn resolved_base_url(&self, config: &ConnectorConfig) -> String {
        config
            .auth_config_json
            .get("api_base_url")
            .and_then(serde_json::Value::as_str)
            .map_or_else(
                || self.api_base_url.clone(),
                std::string::ToString::to_string,
            )
    }

    fn resolved_api_version(&self, config: &ConnectorConfig) -> String {
        config
            .auth_config_json
            .get("api_version")
            .and_then(serde_json::Value::as_str)
            .map_or_else(
                || self.api_version.clone(),
                std::string::ToString::to_string,
            )
    }

    /// Read a required string field from `auth_config_json`.
    fn required_field(config: &ConnectorConfig, key: &str) -> Result<String> {
        config
            .auth_config_json
            .get(key)
            .and_then(serde_json::Value::as_str)
            .map(std::string::ToString::to_string)
            .ok_or_else(|| {
                ConnectorError::Sync(format!(
                    "teams: auth_config_json.{key} is required (team_id + channel_id identify the channel to ingest)"
                ))
            })
    }

    /// `…/teams/{team}/channels/{channel}` prefix.
    fn channel_prefix(&self, config: &ConnectorConfig) -> Result<String> {
        let base = self.resolved_base_url(config);
        let version = self.resolved_api_version(config);
        let team = percent_encode_path_component(&Self::required_field(config, "team_id")?);
        let channel = percent_encode_path_component(&Self::required_field(config, "channel_id")?);
        Ok(format!("{base}{version}/teams/{team}/channels/{channel}"))
    }

    /// Walk every `@odata.nextLink` page of a messages delta feed.
    fn paginate_delta(
        &self,
        first_url: &str,
        token: &OAuth2Token,
    ) -> Result<(Vec<ChatMessage>, Option<String>)> {
        let mut items = Vec::<ChatMessage>::new();
        let mut url = first_url.to_string();
        let mut prev_url: Option<String> = None;
        let mut delta_link: Option<String> = None;
        for _ in 0..MAX_DELTA_PAGES {
            let resp: MessageDeltaResponse = bearer_get_json(
                &self.transport,
                "teams",
                "/teams/{id}/channels/{id}/messages/delta",
                &url,
                token,
                &[],
            )?;
            let returned = resp.value.len();
            items.extend(resp.value);
            if resp.delta_link.is_some() {
                delta_link = resp.delta_link;
            }
            let Some(next) = resp.next_link else {
                return Ok((items, delta_link));
            };
            if prev_url.as_deref() == Some(next.as_str()) || returned == 0 {
                return Ok((items, delta_link));
            }
            prev_url = Some(next.clone());
            url = next;
        }
        Err(ConnectorError::Sync(format!(
            "teams messages delta exceeded {MAX_DELTA_PAGES} pages without exhausting cursor"
        )))
    }
}

/// Which sync pass produced this message.
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
enum SyncMode {
    Initial,
    Incremental,
}

fn message_to_event(msg: &ChatMessage, mode: SyncMode) -> ConnectorEvent {
    let occurred_at = msg
        .last_modified_date_time
        .or(msg.created_date_time)
        .unwrap_or_else(Utc::now);
    let id = SourceDocumentId::new(msg.id.clone());
    if msg.deleted_date_time.is_some() {
        return ConnectorEvent::DocumentDeleted {
            document_id: id,
            occurred_at,
        };
    }
    match mode {
        SyncMode::Initial => ConnectorEvent::DocumentCreated {
            document_id: id,
            occurred_at,
        },
        SyncMode::Incremental => ConnectorEvent::DocumentUpdated {
            document_id: id,
            occurred_at,
        },
    }
}

impl Connector for TeamsConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        let auth_code = config
            .auth_config_json
            .get("authorization_code")
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "teams authenticate: auth_config_json.authorization_code is required".into(),
                )
            })?;
        self.oauth.exchange_code(config, auth_code)
    }

    fn initial_sync(&self, config: &ConnectorConfig, token: &OAuth2Token) -> Result<SyncRunResult> {
        let prefix = self.channel_prefix(config)?;
        let url = format!("{prefix}/messages/delta");
        let (msgs, delta_link) = self.paginate_delta(&url, token)?;
        let events: Vec<ConnectorEvent> = msgs
            .iter()
            .map(|m| message_to_event(m, SyncMode::Initial))
            .collect();
        Ok(SyncRunResult {
            events,
            next_cursor: delta_link,
        })
    }

    fn incremental_sync(
        &self,
        _config: &ConnectorConfig,
        token: &OAuth2Token,
        state: &SyncState,
    ) -> Result<SyncRunResult> {
        let delta_url = state.cursor.as_deref().ok_or_else(|| {
            ConnectorError::Sync(
                "teams incremental_sync: missing cursor; \
                 initial_sync must populate @odata.deltaLink first"
                    .into(),
            )
        })?;
        let (msgs, new_delta_link) = self.paginate_delta(delta_url, token)?;
        let events: Vec<ConnectorEvent> = msgs
            .iter()
            .map(|m| message_to_event(m, SyncMode::Incremental))
            .collect();
        let next_cursor = new_delta_link.or_else(|| Some(delta_url.to_string()));
        Ok(SyncRunResult {
            events,
            next_cursor,
        })
    }

    fn fetch_content(
        &self,
        config: &ConnectorConfig,
        token: &OAuth2Token,
        document_id: &SourceDocumentId,
    ) -> Result<FetchedContent> {
        let prefix = self.channel_prefix(config)?;
        let id = document_id.as_str();
        let id_enc = percent_encode_path_component(id);
        let url = format!("{prefix}/messages/{id_enc}");
        let msg: ChatMessage = bearer_get_json(
            &self.transport,
            "teams",
            "/teams/{id}/channels/{id}/messages/{id}",
            &url,
            token,
            &[],
        )?;
        let body = msg.body.unwrap_or_default();
        // Graph `chatMessage` bodies are either `html` or `text` — never
        // markdown. The html path is stripped to plain text, and the text
        // path is already plain text, so both are `text/plain` (matching
        // the other message connectors: Discord, Slack, Email).
        let text = if body.content_type.eq_ignore_ascii_case("html") {
            strip_html(&body.content)
        } else {
            body.content
        };
        let title = msg
            .subject
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| format!("Teams message {id}"));
        let fc = FetchedContent::text(text, "text/plain")
            .with_title(title)
            .with_metadata(serde_json::json!({
                "provider": "teams",
                "message_id": id,
            }));
        Ok(fc)
    }

    fn subscribe_webhook(
        &self,
        config: &ConnectorConfig,
        token: &OAuth2Token,
        callback_url: &str,
    ) -> Result<WebhookSubscription> {
        let base = self.resolved_base_url(config);
        let version = self.resolved_api_version(config);
        let team = Self::required_field(config, "team_id")?;
        let channel = Self::required_field(config, "channel_id")?;
        let url = format!("{base}{version}/subscriptions");
        let client_state = config
            .auth_config_json
            .get("client_state")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("graph-clientstate-secret")
            .to_string();
        let expires_at = Utc::now() + Duration::minutes(DEFAULT_SUBSCRIPTION_TTL_MINUTES);
        let body = serde_json::json!({
            "changeType": "created,updated,deleted",
            "notificationUrl": callback_url,
            "resource": format!("/teams/{team}/channels/{channel}/messages"),
            "expirationDateTime": expires_at.to_rfc3339(),
            "clientState": client_state,
        });
        let resp: GraphSubscriptionResponse = bearer_post_json(
            &self.transport,
            "teams",
            "/subscriptions",
            &url,
            token,
            &[],
            &body,
        )?;
        let mut subscription = WebhookSubscription::new(
            self.instance,
            callback_url,
            WebhookSecret::new(client_state),
            WebhookEventTypes::all(),
            Some(resp.expiration_date_time.unwrap_or(expires_at)),
        );
        subscription.provider_subscription_id = resp.id;
        Ok(subscription)
    }

    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let batch: ChangeNotificationCollection = serde_json::from_slice(body).map_err(|e| {
            ConnectorError::Webhook(format!("teams webhook: malformed notification body: {e}"))
        })?;
        if batch.value.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty Graph changeNotification batch".to_string(),
            ));
        }
        let mut events: Vec<ConnectorEvent> = Vec::with_capacity(batch.value.len());
        for n in batch.value {
            let occurred_at = n.event_time.unwrap_or_else(Utc::now);
            let document_id = SourceDocumentId::new(
                n.resource
                    .rsplit('/')
                    .next()
                    .unwrap_or(&n.resource)
                    .to_string(),
            );
            let event = match n.change_type.as_str() {
                "created" => ConnectorEvent::DocumentCreated {
                    document_id,
                    occurred_at,
                },
                "updated" => ConnectorEvent::DocumentUpdated {
                    document_id,
                    occurred_at,
                },
                "deleted" => ConnectorEvent::DocumentDeleted {
                    document_id,
                    occurred_at,
                },
                _ => continue,
            };
            events.push(event);
        }
        Ok(events)
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use connector_framework::{
        AuthKind, ConnectorKind, HttpMethod, MockHttpTransport, MockResponse,
    };
    use evidence_store::ScopeId;

    struct FixedOAuth;
    impl OAuth2CodeExchange for FixedOAuth {
        fn exchange_code(&self, _config: &ConnectorConfig, _code: &str) -> Result<OAuth2Token> {
            Ok(OAuth2Token::new(
                "teams-access",
                "teams-refresh",
                Utc::now() + Duration::hours(1),
                "ChannelMessage.Read.All",
            ))
        }
    }

    fn oauth() -> Arc<dyn OAuth2CodeExchange> {
        Arc::new(FixedOAuth)
    }

    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Teams, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "authorization_code": "demo-code",
                "api_base_url": "https://api.test/graph",
                "team_id": "T1",
                "channel_id": "C1",
            }))
    }

    fn ok_json(value: &serde_json::Value) -> MockResponse {
        MockResponse::ok_json(serde_json::to_vec(value).unwrap())
    }

    const DELTA_URL: &str = "https://api.test/graph/v1.0/teams/T1/channels/C1/messages/delta";

    #[test]
    fn authenticate_dispatches_to_oauth_exchange() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        assert_eq!(tok.access_token.expose(), "teams-access");
    }

    #[test]
    fn authenticate_requires_authorization_code() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let cfg_no_code =
            ConnectorConfig::new(ConnectorKind::Teams, AuthKind::OAuth2, ScopeId::new_v4());
        let err = c.authenticate(&cfg_no_code).unwrap_err();
        assert!(matches!(err, ConnectorError::Auth(_)));
    }

    #[test]
    fn initial_sync_emits_created_and_seeds_cursor() {
        let transport = Arc::new(MockHttpTransport::new());
        let now = Utc::now();
        transport.expect(
            HttpMethod::Get,
            DELTA_URL,
            ok_json(&serde_json::json!({
                "value": [
                    { "id": "m1", "createdDateTime": now },
                    { "id": "m2", "createdDateTime": now },
                ],
                "@odata.deltaLink": "https://api.test/graph/v1.0/teams/T1/channels/C1/messages/delta?token=D1"
            })),
        );
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let res = c.initial_sync(&cfg(), &tok).unwrap();
        assert_eq!(res.events.len(), 2);
        assert!(res
            .events
            .iter()
            .all(|e| matches!(e, ConnectorEvent::DocumentCreated { .. })));
        assert!(res.next_cursor.unwrap().contains("token=D1"));
    }

    #[test]
    fn initial_sync_requires_team_and_channel() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let cfg_missing =
            ConnectorConfig::new(ConnectorKind::Teams, AuthKind::OAuth2, ScopeId::new_v4())
                .with_auth_config(serde_json::json!({
                    "authorization_code": "demo-code",
                    "api_base_url": "https://api.test/graph",
                }));
        let tok = c.authenticate(&cfg_missing).unwrap();
        let err = c.initial_sync(&cfg_missing, &tok).unwrap_err();
        assert!(matches!(err, ConnectorError::Sync(_)));
    }

    #[test]
    fn incremental_sync_maps_deleted() {
        let transport = Arc::new(MockHttpTransport::new());
        let now = Utc::now();
        let cursor = "https://api.test/graph/v1.0/teams/T1/channels/C1/messages/delta?token=D1";
        transport.expect(
            HttpMethod::Get,
            cursor,
            ok_json(&serde_json::json!({
                "value": [
                    { "id": "m9", "createdDateTime": now, "deletedDateTime": now },
                ],
                "@odata.deltaLink": "https://api.test/graph/v1.0/teams/T1/channels/C1/messages/delta?token=D2"
            })),
        );
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let mut state = SyncState::new(c.instance);
        state.cursor = Some(cursor.into());
        let res = c.incremental_sync(&cfg(), &tok, &state).unwrap();
        assert_eq!(res.events.len(), 1);
        assert!(matches!(
            res.events[0],
            ConnectorEvent::DocumentDeleted { .. }
        ));
    }

    #[test]
    fn incremental_sync_requires_cursor() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let state = SyncState::new(c.instance);
        let err = c.incremental_sync(&cfg(), &tok, &state).unwrap_err();
        assert!(matches!(err, ConnectorError::Sync(_)));
    }

    #[test]
    fn initial_sync_maps_403_to_auth_error() {
        let transport = Arc::new(MockHttpTransport::new());
        transport.expect(
            HttpMethod::Get,
            DELTA_URL,
            MockResponse::status(403, b"forbidden".to_vec()),
        );
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let err = c.initial_sync(&cfg(), &tok).unwrap_err();
        assert!(matches!(err, ConnectorError::Auth(_)));
    }

    #[test]
    fn fetch_content_flattens_html_body() {
        let transport = Arc::new(MockHttpTransport::new());
        transport.expect(
            HttpMethod::Get,
            "https://api.test/graph/v1.0/teams/T1/channels/C1/messages/m1",
            ok_json(&serde_json::json!({
                "id": "m1",
                "subject": "Release notes",
                "body": { "contentType": "html", "content": "<p>Hello <b>world</b></p>" }
            })),
        );
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let fc = c
            .fetch_content(&cfg(), &tok, &SourceDocumentId::new("m1"))
            .unwrap();
        assert_eq!(fc.title.as_deref(), Some("Release notes"));
        let text = String::from_utf8(fc.body.clone()).unwrap();
        assert!(text.contains("Hello"));
        assert!(text.contains("world"));
    }

    #[test]
    fn subscribe_webhook_posts_subscription_and_keeps_id() {
        let transport = Arc::new(MockHttpTransport::new());
        transport.expect(
            HttpMethod::Post,
            "https://api.test/graph/v1.0/subscriptions",
            ok_json(&serde_json::json!({ "id": "sub-7" })),
        );
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://hook.example/teams")
            .unwrap();
        assert_eq!(sub.provider_subscription_id.as_deref(), Some("sub-7"));
    }

    #[test]
    fn webhook_batch_emits_every_event() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let body = serde_json::json!({
            "value": [
                { "resource": "teams/T1/channels/C1/messages/1", "changeType": "created" },
                { "resource": "teams/T1/channels/C1/messages/2", "changeType": "updated" },
                { "resource": "teams/T1/channels/C1/messages/3", "changeType": "deleted" },
            ]
        });
        let evs = c
            .handle_webhook_event(&serde_json::to_vec(&body).unwrap())
            .unwrap();
        assert_eq!(evs.len(), 3);
        assert!(matches!(evs[2], ConnectorEvent::DocumentDeleted { .. }));
    }

    #[test]
    fn webhook_empty_batch_errors() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let err = c.handle_webhook_event(br#"{"value":[]}"#).unwrap_err();
        assert!(matches!(err, ConnectorError::Webhook(_)));
    }

    #[test]
    fn webhook_malformed_body_errors() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = TeamsConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let err = c.handle_webhook_event(b"not json").unwrap_err();
        assert!(matches!(err, ConnectorError::Webhook(_)));
    }
}
