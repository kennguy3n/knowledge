//! Personio connector — Personio partner API (`https://api.personio.de`).
//!
...
//! `X-Personio-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Personio API base URL (host-only; request paths add `/v1`).
pub const DEFAULT_API_BASE_URL: &str = "https://api.personio.de";
...
/// `X-Personio-Api-Key` header) from an OAuth-issued bearer token.
...
struct PersonioPage {
    #[serde(default)]
    data: Vec<PersonioRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct PersonioRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl std::fmt::Debug for PersonioConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PersonioConnector")
...
impl PersonioConnector {
    /// Construct a Personio connector.
    pub fn new(
    ...
    /// Override the Personio API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("personio", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "personio {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<PersonioRecord>> {
        let mut records = Vec::<PersonioRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: PersonioPage = self.http_get("/v1/employees", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "personio /v1/employees exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Personio-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Personio-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &PersonioRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for PersonioConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "personio authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn subscribe_webhook(
        ...
        // Personio webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("personio-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<PersonioWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<PersonioWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<PersonioWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("personio webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty personio webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.employee_id).ok_or_else(|| {
                ConnectorError::Webhook("personio webhook event missing employee_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Personio, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "personio-key",
                "api_base_url": "https://api.test/personio",
                "webhook_secret": "personio-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Personio, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = PersonioConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "personio-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/personio/v1/employees?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = PersonioConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Personio-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/personio/v1/employees?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Personio-Api-Key") && v == "personio-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/personio/v1/employees?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/personio/v1/employees?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/personio/v1/employees/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Personio employee o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/personio")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "personio-secret");
    }
    ...
    fn default_base_url_has_no_duplicate_version() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!("{DEFAULT_API_BASE_URL}/v1/employees?limit=2&offset=0"),
            ok_json(&serde_json::json!({ "data": [] })),
        );
        let c = PersonioConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        let config =
            ConnectorConfig::new(ConnectorKind::Personio, AuthKind::ApiKey, ScopeId::new_v4())
                .with_auth_config(serde_json::json!({ "api_key": "personio-key" }));
