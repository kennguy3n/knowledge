...
impl StarlingConnector {
    ...
    fn paginate_records(
        ...
        for page in 0..MAX_PAGES {
            ...
            let resp: StarlingPage = self.http_get("/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "starling /transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
impl Connector for StarlingConnector {
    ...
    fn fetch_content(
        ...
        let record: StarlingRecord = self.http_get("/transactions/{id}", &url, token)?;
...
mod tests {
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/starling/transactions?limit=2&offset=0",
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/starling/transactions?limit=2&offset=2",
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/starling/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/starling/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/starling/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
