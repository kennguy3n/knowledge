//! Klara connector — Klara partner API (`https://api.klara.ch`).
//!
...
//! `X-Klara-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Klara API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.klara.ch";
...
/// `X-Klara-Api-Key` header) from an OAuth-issued bearer token.
...
impl std::fmt::Debug for KlaraConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KlaraConnector")
...
impl KlaraConnector {
    ...
    /// Override the Klara API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("klara", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "klara {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<KlaraRecord>> {
        let mut records = Vec::<KlaraRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: KlaraPage = self.http_get("/v1/invoices", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "klara /v1/invoices exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Klara-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Klara-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &KlaraRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for KlaraConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "klara authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn subscribe_webhook(
        ...
        // Klara webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("klara-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<KlaraWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<KlaraWebhookEvent>>(body)
        {
            batch
        } else {
            vec![serde_json::from_slice::<KlaraWebhookEvent>(body)
                .map_err(|e| ConnectorError::Webhook(format!("klara webhook parse failed: {e}")))?]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty klara webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.invoice_id).ok_or_else(|| {
                ConnectorError::Webhook("klara webhook event missing invoice_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Klara, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "klara-key",
                "api_base_url": "https://api.test/klara",
                "webhook_secret": "klara-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Klara, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = KlaraConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "klara-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/klara/v1/invoices?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = KlaraConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Klara-Api-Key")));
    }
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = KlaraConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::Klara, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/klara/v1/invoices?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Klara-Api-Key") && v == "klara-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/klara/v1/invoices?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/klara/v1/invoices?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/klara/v1/invoices/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Klara invoice o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/klara")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "klara-secret");
    }
