//! Deputy connector — Deputy partner API (`https://api.deputy.com/api/v1`).
//!
...
//! `X-Deputy-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/timesheets`
...
//! * `fetch_content` GETs a single timesheet (`/v1/timesheets/{id}`).
...
/// Default Deputy API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.deputy.com/api/v1";
/// Default scope recorded on the synthesised API-key token.
pub const DEFAULT_SCOPE: &str = "timesheets";
/// `OAuth2Token::token_type` marker for a static API-key credential.
/// Distinguishes the API-key auth path (provider-native
/// `X-Deputy-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for timesheet listing (`limit`).
...
#[derive(Debug, Clone, Default, Deserialize)]
struct DeputyWebhookEvent {
    #[serde(default)]
    timesheet_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// Deputy connector.
pub struct DeputyConnector {
...
impl std::fmt::Debug for DeputyConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DeputyConnector")
...
impl DeputyConnector {
    /// Construct a Deputy connector.
    pub fn new(
    ...
    /// Override the Deputy API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("deputy", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "deputy {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<DeputyRecord>> {
        let mut records = Vec::<DeputyRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/timesheets?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: DeputyPage = self.http_get("/v1/timesheets", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "deputy /v1/timesheets exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Deputy-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Deputy-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &DeputyRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for DeputyConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "deputy authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/timesheets/{id_enc}");
        let record: DeputyRecord = self.http_get("/v1/timesheets/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Deputy timesheet {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Deputy timesheet {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Deputy webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("deputy-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<DeputyWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<DeputyWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<DeputyWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("deputy webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty deputy webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.timesheet_id).ok_or_else(|| {
                ConnectorError::Webhook("deputy webhook event missing timesheet_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Deputy, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "deputy-key",
                "api_base_url": "https://api.test/deputy",
                "webhook_secret": "deputy-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Deputy, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = DeputyConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "deputy-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deputy/v1/timesheets?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = DeputyConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Deputy-Api-Key")));
    }
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = DeputyConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::Deputy, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deputy/v1/timesheets?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Deputy-Api-Key") && v == "deputy-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/deputy/v1/timesheets?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/deputy/v1/timesheets?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deputy/v1/timesheets/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Deputy timesheet o-1"));
        assert!(body.contains("Sample timesheet"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/deputy")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "deputy-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "timesheet_id": 42, "event": "timesheet.updated" }),
        )
        .unwrap();
