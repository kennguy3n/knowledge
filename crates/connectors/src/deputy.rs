...
//!
//! * `initial_sync` / `incremental_sync` page `/timesheets`
...
//! * `fetch_content` GETs a single timesheet (`/timesheets/{id}`).
...
impl DeputyConnector {
    ...
    fn paginate_records(
        ...
        for page in 0..MAX_PAGES {
            ...
            let mut url = format!(
                "{base_url}/timesheets?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: DeputyPage = self.http_get("/timesheets", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "deputy /timesheets exceeded {MAX_PAGES} pages"
        )))
    }
}
...
impl Connector for DeputyConnector {
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/timesheets/{id_enc}");
        let record: DeputyRecord = self.http_get("/timesheets/{id}", &url, token)?;
...
mod tests {
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deputy/timesheets?limit=2&offset=0",
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deputy/timesheets?limit=2&offset=2",
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/deputy/timesheets?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/deputy/timesheets?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deputy/timesheets/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
