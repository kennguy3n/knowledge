//! Employment Hero connector — Employment Hero partner API (`https://api.employmenthero.com/api/v1`).
//!
...
//! `X-EmploymentHero-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/employees`
...
//! * `fetch_content` GETs a single employee (`/v1/employees/{id}`).
...
/// Default Employment Hero API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.employmenthero.com/api/v1";
...
/// `X-EmploymentHero-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for employee listing (`limit`).
pub const DEFAULT_PAGE_SIZE: u32 = 100;
/// Safety ceiling on the number of pages a single sync walks.
pub const MAX_PAGES: usize = 100_000;

#[derive(Debug, Clone, Default, Deserialize)]
struct EmploymentHeroPage {
    #[serde(default)]
    data: Vec<EmploymentHeroRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct EmploymentHeroRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct EmploymentHeroWebhookEvent {
    #[serde(default)]
...
pub struct EmploymentHeroConnector {
...
impl std::fmt::Debug for EmploymentHeroConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EmploymentHeroConnector")
...
impl EmploymentHeroConnector {
    /// Construct a Employment Hero connector.
    pub fn new(
    ...
    /// Override the Employment Hero API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("employment_hero", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "employment_hero {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<EmploymentHeroRecord>> {
        let mut records = Vec::<EmploymentHeroRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/employees?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: EmploymentHeroPage = self.http_get("/v1/employees", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "employment_hero /v1/employees exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-EmploymentHero-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-EmploymentHero-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &EmploymentHeroRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for EmploymentHeroConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "employment_hero authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/employees/{id_enc}");
        let record: EmploymentHeroRecord = self.http_get("/v1/employees/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Employment Hero employee {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Employment Hero employee {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Employment Hero webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("employment_hero-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<EmploymentHeroWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<EmploymentHeroWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<EmploymentHeroWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("employment_hero webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty employment_hero webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.employee_id).ok_or_else(|| {
                ConnectorError::Webhook("employment_hero webhook event missing employee_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "employment_hero-key",
            "api_base_url": "https://api.test/employment_hero",
            "webhook_secret": "employment_hero-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = EmploymentHeroConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "employment_hero-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/employment_hero/v1/employees?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            EmploymentHeroConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-EmploymentHero-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/employment_hero/v1/employees?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-EmploymentHero-Api-Key")
                && v == "employment_hero-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/employment_hero/v1/employees?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/employment_hero/v1/employees?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/employment_hero/v1/employees/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Employment Hero employee o-1"));
        assert!(body.contains("Sample employee"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/employment_hero",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "employment_hero-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "employee_id": 42, "event": "employee.updated" }),
        )
        .unwrap();
