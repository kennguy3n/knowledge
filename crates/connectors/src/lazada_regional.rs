//! Lazada Regional connector — Lazada Regional partner API (`https://api.lazada.com/rest`).
//!
...
//! `X-LazadaRegional-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Lazada Regional API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.lazada.com/rest";
...
/// `X-LazadaRegional-Api-Key` header) from an OAuth-issued bearer token.
...
struct LazadaRegionalPage {
    #[serde(default)]
    data: Vec<LazadaRegionalRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct LazadaRegionalRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct LazadaRegionalWebhookEvent {
    #[serde(default)]
...
/// Lazada Regional connector.
pub struct LazadaRegionalConnector {
...
impl std::fmt::Debug for LazadaRegionalConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LazadaRegionalConnector")
...
impl LazadaRegionalConnector {
    /// Construct a Lazada Regional connector.
    pub fn new(
    ...
    /// Override the Lazada Regional API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("lazada_regional", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "lazada_regional {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<LazadaRegionalRecord>> {
        let mut records = Vec::<LazadaRegionalRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: LazadaRegionalPage = self.http_get("/v1/orders", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "lazada_regional /v1/orders exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-LazadaRegional-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-LazadaRegional-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &LazadaRegionalRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for LazadaRegionalConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "lazada_regional authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: LazadaRegionalRecord = self.http_get("/v1/orders/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Lazada Regional order {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Lazada Regional order {id}"))
            .with_metadata(serde_json::json!({
                "provider": "lazada_regional",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // Lazada Regional webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("lazada_regional-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<LazadaRegionalWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<LazadaRegionalWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<LazadaRegionalWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("lazada_regional webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty lazada_regional webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.order_id).ok_or_else(|| {
                ConnectorError::Webhook("lazada_regional webhook event missing order_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "lazada_regional-key",
            "api_base_url": "https://api.test/lazada_regional",
            "webhook_secret": "lazada_regional-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        assert_eq!(token.access_token.expose(), "lazada_regional-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/lazada_regional/v1/orders?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            LazadaRegionalConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-LazadaRegional-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/lazada_regional/v1/orders?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-LazadaRegional-Api-Key")
                && v == "lazada_regional-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/lazada_regional/v1/orders?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/lazada_regional/v1/orders?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/lazada_regional/v1/orders/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample order"
        ...
        assert!(body.contains("# Lazada Regional order o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/lazada_regional",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "lazada_regional-secret");
    }
