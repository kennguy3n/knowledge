//! Correos de México connector — Correos de México partner API (`https://api.correosdemexico.gob.mx`).
//!
...
//! `X-CorreosMexico-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/shipments`
...
//! * `fetch_content` GETs a single shipment (`/v1/shipments/{id}`).
...
/// Default Correos de México API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.correosdemexico.gob.mx";
/// Default scope recorded on the synthesised API-key token.
pub const DEFAULT_SCOPE: &str = "shipments";
/// `OAuth2Token::token_type` marker for a static API-key credential.
/// Distinguishes the API-key auth path (provider-native
/// `X-CorreosMexico-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for shipment listing (`limit`).
pub const DEFAULT_PAGE_SIZE: u32 = 100;
/// Safety ceiling on the number of pages a single sync walks.
pub const MAX_PAGES: usize = 100_000;

#[derive(Debug, Clone, Default, Deserialize)]
struct CorreosMexicoPage {
    #[serde(default)]
    data: Vec<CorreosMexicoRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct CorreosMexicoRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct CorreosMexicoWebhookEvent {
    #[serde(default)]
    shipment_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// Correos de México connector.
pub struct CorreosMexicoConnector {
...
impl std::fmt::Debug for CorreosMexicoConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CorreosMexicoConnector")
...
impl CorreosMexicoConnector {
    /// Construct a Correos de México connector.
    pub fn new(
    ...
    /// Override the Correos de México API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("correos_mexico", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "correos_mexico {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<CorreosMexicoRecord>> {
        let mut records = Vec::<CorreosMexicoRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/shipments?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: CorreosMexicoPage = self.http_get("/v1/shipments", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "correos_mexico /v1/shipments exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-CorreosMexico-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-CorreosMexico-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &CorreosMexicoRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for CorreosMexicoConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "correos_mexico authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: CorreosMexicoRecord = self.http_get("/v1/shipments/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body =
            format!("# Correos de México shipment {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Correos de México shipment {id}"))
            .with_metadata(serde_json::json!({
                "provider": "correos_mexico",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // Correos de México webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("correos_mexico-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<CorreosMexicoWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<CorreosMexicoWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<CorreosMexicoWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("correos_mexico webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty correos_mexico webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.shipment_id).ok_or_else(|| {
                ConnectorError::Webhook("correos_mexico webhook event missing shipment_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "correos_mexico-key",
            "api_base_url": "https://api.test/correos_mexico",
            "webhook_secret": "correos_mexico-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = CorreosMexicoConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "correos_mexico-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/correos_mexico/v1/shipments?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            CorreosMexicoConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-CorreosMexico-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/correos_mexico/v1/shipments?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-CorreosMexico-Api-Key")
                && v == "correos_mexico-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/correos_mexico/v1/shipments?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/correos_mexico/v1/shipments?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/correos_mexico/v1/shipments/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Correos de México shipment o-1"));
        assert!(body.contains("Sample shipment"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/correos_mexico",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "correos_mexico-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "shipment_id": 42, "event": "shipment.updated" }),
        )
        .unwrap();
