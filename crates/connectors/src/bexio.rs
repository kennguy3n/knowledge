...
/// `X-Bexio-Api-Key` header) from an OAuth-issued bearer token.
...
impl std::fmt::Debug for BexioConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BexioConnector")
...
impl BexioConnector {
    ...
    /// Override the Bexio API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("bexio", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "bexio {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    ) -> Result<Vec<BexioRecord>> {
        let mut records = Vec::<BexioRecord>::new();
...
/// goes in the provider-native `X-Bexio-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Bexio-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &BexioRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for BexioConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "bexio authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Bexio invoice {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Bexio webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("bexio-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<BexioWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<BexioWebhookEvent>>(body)
        {
            batch
        } else {
            vec![serde_json::from_slice::<BexioWebhookEvent>(body)
                .map_err(|e| ConnectorError::Webhook(format!("bexio webhook parse failed: {e}")))?]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty bexio webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.invoice_id).ok_or_else(|| {
                ConnectorError::Webhook("bexio webhook event missing invoice_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Bexio, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "bexio-key",
                "api_base_url": "https://api.test/bexio",
                "webhook_secret": "bexio-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Bexio, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        assert_eq!(token.access_token.expose(), "bexio-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Bexio-Api-Key")));
    }
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = BexioConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::Bexio, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Bexio-Api-Key") && v == "bexio-key"));
    }
    ...
    fn fetch_content_renders_markdown() {
        ...
        assert!(body.contains("# Bexio invoice o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/bexio")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "bexio-secret");
    }
