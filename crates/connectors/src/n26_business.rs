//! N26 Business connector — N26 Business partner API (`https://api.tech26.de`).
//!
...
//! `X-N26Business-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default N26 Business API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.tech26.de";
...
/// `X-N26Business-Api-Key` header) from an OAuth-issued bearer token.
...
struct N26BusinessPage {
    #[serde(default)]
    data: Vec<N26BusinessRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct N26BusinessRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct N26BusinessWebhookEvent {
    #[serde(default)]
...
/// N26 Business connector.
pub struct N26BusinessConnector {
...
impl std::fmt::Debug for N26BusinessConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("N26BusinessConnector")
...
impl N26BusinessConnector {
    /// Construct a N26 Business connector.
    pub fn new(
    ...
    /// Override the N26 Business API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("n26_business", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "n26_business {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<N26BusinessRecord>> {
        let mut records = Vec::<N26BusinessRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: N26BusinessPage = self.http_get("/v1/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "n26_business /v1/transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-N26Business-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-N26Business-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &N26BusinessRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for N26BusinessConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "n26_business authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: N26BusinessRecord = self.http_get("/v1/transactions/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# N26 Business transaction {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("N26 Business transaction {id}"))
            .with_metadata(serde_json::json!({
                "provider": "n26_business",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // N26 Business webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("n26_business-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<N26BusinessWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<N26BusinessWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<N26BusinessWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("n26_business webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty n26_business webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.transaction_id).ok_or_else(|| {
                ConnectorError::Webhook("n26_business webhook event missing transaction_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "n26_business-key",
            "api_base_url": "https://api.test/n26_business",
            "webhook_secret": "n26_business-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = N26BusinessConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "n26_business-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/n26_business/v1/transactions?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            N26BusinessConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-N26Business-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/n26_business/v1/transactions?limit=2&offset=2",
        ...
        assert!(recorded[0].headers.iter().any(|(k, v)| k
            .eq_ignore_ascii_case("X-N26Business-Api-Key")
            && v == "n26_business-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/n26_business/v1/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/n26_business/v1/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/n26_business/v1/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# N26 Business transaction o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/n26_business",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "n26_business-secret");
    }
