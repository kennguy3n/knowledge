//! Digitec Galaxus connector — Digitec Galaxus partner API (`https://api.galaxus.ch`).
//!
...
//! `X-DigitecGalaxus-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Digitec Galaxus API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.galaxus.ch";
...
/// `X-DigitecGalaxus-Api-Key` header) from an OAuth-issued bearer token.
...
struct DigitecGalaxusPage {
    #[serde(default)]
    data: Vec<DigitecGalaxusRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct DigitecGalaxusRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct DigitecGalaxusWebhookEvent {
    #[serde(default)]
...
/// Digitec Galaxus connector.
pub struct DigitecGalaxusConnector {
...
impl std::fmt::Debug for DigitecGalaxusConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DigitecGalaxusConnector")
...
impl DigitecGalaxusConnector {
    /// Construct a Digitec Galaxus connector.
    pub fn new(
    ...
    /// Override the Digitec Galaxus API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("digitec_galaxus", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "digitec_galaxus {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<DigitecGalaxusRecord>> {
        let mut records = Vec::<DigitecGalaxusRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: DigitecGalaxusPage = self.http_get("/v1/orders", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "digitec_galaxus /v1/orders exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-DigitecGalaxus-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-DigitecGalaxus-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &DigitecGalaxusRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for DigitecGalaxusConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "digitec_galaxus authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: DigitecGalaxusRecord = self.http_get("/v1/orders/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Digitec Galaxus order {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Digitec Galaxus order {id}"))
            .with_metadata(serde_json::json!({
                "provider": "digitec_galaxus",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // Digitec Galaxus webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("digitec_galaxus-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<DigitecGalaxusWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<DigitecGalaxusWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<DigitecGalaxusWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("digitec_galaxus webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty digitec_galaxus webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.order_id).ok_or_else(|| {
                ConnectorError::Webhook("digitec_galaxus webhook event missing order_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "digitec_galaxus-key",
            "api_base_url": "https://api.test/digitec_galaxus",
            "webhook_secret": "digitec_galaxus-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = DigitecGalaxusConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "digitec_galaxus-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/digitec_galaxus/v1/orders?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            DigitecGalaxusConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-DigitecGalaxus-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/digitec_galaxus/v1/orders?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-DigitecGalaxus-Api-Key")
                && v == "digitec_galaxus-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/digitec_galaxus/v1/orders?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/digitec_galaxus/v1/orders?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/digitec_galaxus/v1/orders/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample order"
        ...
        assert!(body.contains("# Digitec Galaxus order o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/digitec_galaxus",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "digitec_galaxus-secret");
    }
