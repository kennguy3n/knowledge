//! MyEG connector — MyEG partner API (`https://api.myeg.com.my`).
//!
...
//! `X-MyEG-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/services`
...
//! * `fetch_content` GETs a single service (`/v1/services/{id}`).
...
/// Default MyEG API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.myeg.com.my";
...
/// `X-MyEG-Api-Key` header) from an OAuth-issued bearer token.
...
impl std::fmt::Debug for MyEGConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MyEGConnector")
...
impl MyEGConnector {
    ...
    /// Override the MyEG API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("myeg", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "myeg {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<MyEGRecord>> {
        let mut records = Vec::<MyEGRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/services?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: MyEGPage = self.http_get("/v1/services", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "myeg /v1/services exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-MyEG-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-MyEG-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &MyEGRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for MyEGConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "myeg authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/services/{id_enc}");
        let record: MyEGRecord = self.http_get("/v1/services/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# MyEG service {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("MyEG service {id}"))
    ...
    fn subscribe_webhook(
        ...
        // MyEG webhooks are registered in the provider
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<MyEGWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<MyEGWebhookEvent>>(body)
        {
            batch
        } else {
            vec![serde_json::from_slice::<MyEGWebhookEvent>(body)
                .map_err(|e| ConnectorError::Webhook(format!("myeg webhook parse failed: {e}")))?]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty myeg webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.service_id).ok_or_else(|| {
                ConnectorError::Webhook("myeg webhook event missing service_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::MyEG, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "myeg-key",
                "api_base_url": "https://api.test/myeg",
                "webhook_secret": "myeg-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::MyEG, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = MyEGConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "myeg-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/myeg/v1/services?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = MyEGConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-MyEG-Api-Key")));
    }
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = MyEGConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::MyEG, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/myeg/v1/services?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-MyEG-Api-Key") && v == "myeg-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/myeg/v1/services?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/myeg/v1/services?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
            ok_json(&serde_json::json!({ "data": [ {"id": "o-11", "updated_at": "2024-06-01T00:00:00Z"} ] })),
        );
        let c =
            MyEGConnector::new(ConnectorInstanceId::new_v4(), transport, oauth()).with_page_size(2);
        let tok = c.authenticate(&cfg()).unwrap();
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/myeg/v1/services/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# MyEG service o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/myeg")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "myeg-secret");
    }
    ...
    fn handle_webhook_event_maps_event_kinds() {
        ...
        let body = serde_json::to_vec(&serde_json::json!([
            { "service_id": "x-1", "event": "service.created" },
            { "service_id": "x-2", "event": "service.cancelled" },
            { "service_id": "x-3", "event": "service.deleted" },
            { "service_id": "x-4", "event": "service.updated" }
        ]))
        .unwrap();
        let events = c.handle_webhook_event(&body).unwrap();
