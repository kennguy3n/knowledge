//! Campaign Monitor connector — Campaign Monitor partner API (`https://api.createsend.com/api/v3.3`).
//!
...
//! `X-CampaignMonitor-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/subscribers`
...
//! * `fetch_content` GETs a single subscriber (`/v1/subscribers/{id}`).
...
/// Default Campaign Monitor API base URL.
...
#[derive(Debug, Clone, Default, Deserialize)]
struct CampaignMonitorWebhookEvent {
    #[serde(default)]
    subscriber_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// Campaign Monitor connector.
pub struct CampaignMonitorConnector {
...
impl std::fmt::Debug for CampaignMonitorConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CampaignMonitorConnector")
...
impl CampaignMonitorConnector {
    /// Construct a Campaign Monitor connector.
    pub fn new(
    ...
    /// Override the Campaign Monitor API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("campaign_monitor", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "campaign_monitor {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<CampaignMonitorRecord>> {
        let mut records = Vec::<CampaignMonitorRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/subscribers?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: CampaignMonitorPage = self.http_get("/v1/subscribers", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "campaign_monitor /v1/subscribers exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-CampaignMonitor-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-CampaignMonitor-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &CampaignMonitorRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for CampaignMonitorConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "campaign_monitor authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/subscribers/{id_enc}");
        let record: CampaignMonitorRecord = self.http_get("/v1/subscribers/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body =
            format!("# Campaign Monitor subscriber {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Campaign Monitor subscriber {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Campaign Monitor webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("campaign_monitor-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<CampaignMonitorWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<CampaignMonitorWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<CampaignMonitorWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("campaign_monitor webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty campaign_monitor webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.subscriber_id).ok_or_else(|| {
                ConnectorError::Webhook(
                    "campaign_monitor webhook event missing subscriber_id".into(),
                )
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "campaign_monitor-key",
            "api_base_url": "https://api.test/campaign_monitor",
            "webhook_secret": "campaign_monitor-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = CampaignMonitorConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "campaign_monitor-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/campaign_monitor/v1/subscribers?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = CampaignMonitorConnector::new(
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-CampaignMonitor-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/campaign_monitor/v1/subscribers?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-CampaignMonitor-Api-Key")
                && v == "campaign_monitor-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/campaign_monitor/v1/subscribers?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/campaign_monitor/v1/subscribers?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/campaign_monitor/v1/subscribers/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample subscriber"
            })),
        );
        let c = CampaignMonitorConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        ...
        assert!(body.contains("# Campaign Monitor subscriber o-1"));
        assert!(body.contains("Sample subscriber"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/campaign_monitor",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "campaign_monitor-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "subscriber_id": 42, "event": "subscriber.updated" }),
        )
        .unwrap();
