//! SeaMoney connector — SeaMoney partner API (`https://api.seamoney.com`).
//!
...
//! `X-SeaMoney-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/payments`
...
//! * `fetch_content` GETs a single payment (`/v1/payments/{id}`).
...
/// Default SeaMoney API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.seamoney.com";
/// Default scope recorded on the synthesised API-key token.
pub const DEFAULT_SCOPE: &str = "payments";
/// `OAuth2Token::token_type` marker for a static API-key credential.
/// Distinguishes the API-key auth path (provider-native
/// `X-SeaMoney-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for payment listing (`limit`).
...
struct SeaMoneyPage {
    #[serde(default)]
    data: Vec<SeaMoneyRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct SeaMoneyRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl std::fmt::Debug for SeaMoneyConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SeaMoneyConnector")
...
impl SeaMoneyConnector {
    /// Construct a SeaMoney connector.
    pub fn new(
    ...
    /// Override the SeaMoney API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("sea_money", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "sea_money {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<SeaMoneyRecord>> {
        let mut records = Vec::<SeaMoneyRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/payments?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: SeaMoneyPage = self.http_get("/v1/payments", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "sea_money /v1/payments exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-SeaMoney-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-SeaMoney-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &SeaMoneyRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for SeaMoneyConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "sea_money authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/payments/{id_enc}");
        let record: SeaMoneyRecord = self.http_get("/v1/payments/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# SeaMoney payment {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("SeaMoney payment {id}"))
    ...
    fn subscribe_webhook(
        ...
        // SeaMoney webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("sea_money-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<SeaMoneyWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<SeaMoneyWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<SeaMoneyWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("sea_money webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty sea_money webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.payment_id).ok_or_else(|| {
                ConnectorError::Webhook("sea_money webhook event missing payment_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::SeaMoney, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "sea_money-key",
                "api_base_url": "https://api.test/sea_money",
                "webhook_secret": "sea_money-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::SeaMoney, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = SeaMoneyConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "sea_money-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/sea_money/v1/payments?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = SeaMoneyConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-SeaMoney-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/sea_money/v1/payments?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-SeaMoney-Api-Key") && v == "sea_money-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/sea_money/v1/payments?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/sea_money/v1/payments?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/sea_money/v1/payments/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# SeaMoney payment o-1"));
        assert!(body.contains("Sample payment"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/sea_money")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "sea_money-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "payment_id": 42, "event": "payment.updated" }),
        )
        .unwrap();
    ...
    fn handle_webhook_event_maps_event_kinds() {
        ...
        let body = serde_json::to_vec(&serde_json::json!([
            { "payment_id": "x-1", "event": "payment.created" },
            { "payment_id": "x-2", "event": "payment.cancelled" },
            { "payment_id": "x-3", "event": "payment.deleted" },
            { "payment_id": "x-4", "event": "payment.updated" }
        ]))
        .unwrap();
        let events = c.handle_webhook_event(&body).unwrap();
