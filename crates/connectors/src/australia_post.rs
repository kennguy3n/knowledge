//! Australia Post connector — Australia Post partner API (`https://digitalapi.auspost.com.au`).
//!
...
//! `X-AustraliaPost-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Australia Post API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://digitalapi.auspost.com.au";
...
/// `X-AustraliaPost-Api-Key` header) from an OAuth-issued bearer token.
...
struct AustraliaPostPage {
    #[serde(default)]
    data: Vec<AustraliaPostRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct AustraliaPostRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct AustraliaPostWebhookEvent {
    #[serde(default)]
...
pub struct AustraliaPostConnector {
...
impl std::fmt::Debug for AustraliaPostConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AustraliaPostConnector")
...
impl AustraliaPostConnector {
    /// Construct a Australia Post connector.
    pub fn new(
    ...
    /// Override the Australia Post API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("australia_post", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "australia_post {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<AustraliaPostRecord>> {
        let mut records = Vec::<AustraliaPostRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: AustraliaPostPage = self.http_get("/v1/shipments", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "australia_post /v1/shipments exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-AustraliaPost-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-AustraliaPost-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &AustraliaPostRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for AustraliaPostConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "australia_post authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: AustraliaPostRecord = self.http_get("/v1/shipments/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Australia Post shipment {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Australia Post shipment {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Australia Post webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("australia_post-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<AustraliaPostWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<AustraliaPostWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<AustraliaPostWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("australia_post webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty australia_post webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.shipment_id).ok_or_else(|| {
                ConnectorError::Webhook("australia_post webhook event missing shipment_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "australia_post-key",
            "api_base_url": "https://api.test/australia_post",
            "webhook_secret": "australia_post-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = AustraliaPostConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "australia_post-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/australia_post/v1/shipments?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            AustraliaPostConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-AustraliaPost-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/australia_post/v1/shipments?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-AustraliaPost-Api-Key")
                && v == "australia_post-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/australia_post/v1/shipments?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/australia_post/v1/shipments?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/australia_post/v1/shipments/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Australia Post shipment o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/australia_post",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "australia_post-secret");
    }
