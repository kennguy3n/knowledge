//! Ricardo connector — Ricardo partner API (`https://api.ricardo.ch`).
//!
...
//! `X-Ricardo-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/listings`
...
//! * `fetch_content` GETs a single listing (`/v1/listings/{id}`).
...
/// Default Ricardo API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.ricardo.ch";
/// Default scope recorded on the synthesised API-key token.
pub const DEFAULT_SCOPE: &str = "listings";
/// `OAuth2Token::token_type` marker for a static API-key credential.
/// Distinguishes the API-key auth path (provider-native
/// `X-Ricardo-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for listing listing (`limit`).
...
struct RicardoPage {
    #[serde(default)]
    data: Vec<RicardoRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct RicardoRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl std::fmt::Debug for RicardoConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RicardoConnector")
...
impl RicardoConnector {
    /// Construct a Ricardo connector.
    pub fn new(
    ...
    /// Override the Ricardo API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("ricardo", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "ricardo {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<RicardoRecord>> {
        let mut records = Vec::<RicardoRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/listings?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: RicardoPage = self.http_get("/v1/listings", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "ricardo /v1/listings exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Ricardo-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Ricardo-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &RicardoRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for RicardoConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "ricardo authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/listings/{id_enc}");
        let record: RicardoRecord = self.http_get("/v1/listings/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Ricardo listing {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Ricardo listing {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Ricardo webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("ricardo-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<RicardoWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<RicardoWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<RicardoWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("ricardo webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty ricardo webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.listing_id).ok_or_else(|| {
                ConnectorError::Webhook("ricardo webhook event missing listing_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Ricardo, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "ricardo-key",
                "api_base_url": "https://api.test/ricardo",
                "webhook_secret": "ricardo-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Ricardo, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = RicardoConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "ricardo-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/ricardo/v1/listings?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = RicardoConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Ricardo-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/ricardo/v1/listings?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Ricardo-Api-Key") && v == "ricardo-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/ricardo/v1/listings?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/ricardo/v1/listings?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/ricardo/v1/listings/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Ricardo listing o-1"));
        assert!(body.contains("Sample listing"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/ricardo")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "ricardo-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "listing_id": 42, "event": "listing.updated" }),
        )
        .unwrap();
