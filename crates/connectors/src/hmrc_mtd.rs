//! HMRC MTD connector — HMRC MTD partner API (`https://api.service.hmrc.gov.uk`).
//!
...
//! `X-HmrcMtd-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/obligations`
...
//! * `fetch_content` GETs a single obligation (`/v1/obligations/{id}`).
...
/// Default HMRC MTD API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.service.hmrc.gov.uk";
/// Default scope recorded on the synthesised API-key token.
pub const DEFAULT_SCOPE: &str = "obligations";
/// `OAuth2Token::token_type` marker for a static API-key credential.
/// Distinguishes the API-key auth path (provider-native
/// `X-HmrcMtd-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for obligation listing (`limit`).
...
struct HmrcMtdPage {
    #[serde(default)]
    data: Vec<HmrcMtdRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct HmrcMtdRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct HmrcMtdWebhookEvent {
    #[serde(default)]
    obligation_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// HMRC MTD connector.
pub struct HmrcMtdConnector {
...
impl std::fmt::Debug for HmrcMtdConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HmrcMtdConnector")
...
impl HmrcMtdConnector {
    /// Construct a HMRC MTD connector.
    pub fn new(
    ...
    /// Override the HMRC MTD API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("hmrc_mtd", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "hmrc_mtd {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<HmrcMtdRecord>> {
        let mut records = Vec::<HmrcMtdRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/obligations?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: HmrcMtdPage = self.http_get("/v1/obligations", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "hmrc_mtd /v1/obligations exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-HmrcMtd-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-HmrcMtd-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &HmrcMtdRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for HmrcMtdConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "hmrc_mtd authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/obligations/{id_enc}");
        let record: HmrcMtdRecord = self.http_get("/v1/obligations/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# HMRC MTD obligation {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("HMRC MTD obligation {id}"))
    ...
    fn subscribe_webhook(
        ...
        // HMRC MTD webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("hmrc_mtd-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<HmrcMtdWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<HmrcMtdWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<HmrcMtdWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("hmrc_mtd webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty hmrc_mtd webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.obligation_id).ok_or_else(|| {
                ConnectorError::Webhook("hmrc_mtd webhook event missing obligation_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::HmrcMtd, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "hmrc_mtd-key",
                "api_base_url": "https://api.test/hmrc_mtd",
                "webhook_secret": "hmrc_mtd-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::HmrcMtd, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = HmrcMtdConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "hmrc_mtd-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/hmrc_mtd/v1/obligations?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = HmrcMtdConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-HmrcMtd-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/hmrc_mtd/v1/obligations?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-HmrcMtd-Api-Key") && v == "hmrc_mtd-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/hmrc_mtd/v1/obligations?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/hmrc_mtd/v1/obligations?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/hmrc_mtd/v1/obligations/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample obligation"
            })),
        );
        let c = HmrcMtdConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        ...
        assert!(body.contains("# HMRC MTD obligation o-1"));
        assert!(body.contains("Sample obligation"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/hmrc_mtd")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "hmrc_mtd-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "obligation_id": 42, "event": "obligation.updated" }),
        )
        .unwrap();
