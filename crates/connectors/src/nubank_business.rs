//! Nubank Business connector — Nubank Business partner API (`https://api.nubank.com.br`).
//!
...
//! `X-NubankBusiness-Api-Key` header (read from `auth_config_json.api_key`),
...
//! * `fetch_content` GETs a single transaction (`/v1/transactions/{id}`).
...
/// Default Nubank Business API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.nubank.com.br";
...
/// `X-NubankBusiness-Api-Key` header) from an OAuth-issued bearer token.
...
struct NubankBusinessPage {
    #[serde(default)]
    data: Vec<NubankBusinessRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct NubankBusinessRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct NubankBusinessWebhookEvent {
    #[serde(default)]
...
/// Nubank Business connector.
pub struct NubankBusinessConnector {
...
impl std::fmt::Debug for NubankBusinessConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NubankBusinessConnector")
...
impl NubankBusinessConnector {
    /// Construct a Nubank Business connector.
    pub fn new(
    ...
    /// Override the Nubank Business API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("nubank_business", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "nubank_business {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<NubankBusinessRecord>> {
        let mut records = Vec::<NubankBusinessRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: NubankBusinessPage = self.http_get("/v1/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "nubank_business /v1/transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-NubankBusiness-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-NubankBusiness-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &NubankBusinessRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for NubankBusinessConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "nubank_business authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: NubankBusinessRecord = self.http_get("/v1/transactions/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body =
            format!("# Nubank Business transaction {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Nubank Business transaction {id}"))
            .with_metadata(serde_json::json!({
                "provider": "nubank_business",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // Nubank Business webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("nubank_business-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<NubankBusinessWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<NubankBusinessWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<NubankBusinessWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("nubank_business webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty nubank_business webhook batch".into(),
            ));
        }
        ...
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.transaction_id).ok_or_else(|| {
                ConnectorError::Webhook(
                    "nubank_business webhook event missing transaction_id".into(),
                )
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "nubank_business-key",
            "api_base_url": "https://api.test/nubank_business",
            "webhook_secret": "nubank_business-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = NubankBusinessConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "nubank_business-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/nubank_business/v1/transactions?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            NubankBusinessConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-NubankBusiness-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/nubank_business/v1/transactions?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-NubankBusiness-Api-Key")
                && v == "nubank_business-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/nubank_business/v1/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/nubank_business/v1/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/nubank_business/v1/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Nubank Business transaction o-1"));
        assert!(body.contains("Sample transaction"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/nubank_business",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "nubank_business-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let body = serde_json::to_vec(
            &serde_json::json!({ "transaction_id": 42, "event": "transaction.updated" }),
        )
        .unwrap();
