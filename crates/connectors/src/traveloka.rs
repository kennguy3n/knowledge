//! Traveloka connector — Traveloka partner API (`https://api.traveloka.com`).
//!
...
//! `X-Traveloka-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Traveloka API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.traveloka.com";
...
/// `X-Traveloka-Api-Key` header) from an OAuth-issued bearer token.
...
struct TravelokaPage {
    #[serde(default)]
    data: Vec<TravelokaRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct TravelokaRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl std::fmt::Debug for TravelokaConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TravelokaConnector")
...
impl TravelokaConnector {
    /// Construct a Traveloka connector.
    pub fn new(
    ...
    /// Override the Traveloka API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("traveloka", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "traveloka {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<TravelokaRecord>> {
        let mut records = Vec::<TravelokaRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: TravelokaPage = self.http_get("/v1/bookings", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "traveloka /v1/bookings exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Traveloka-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Traveloka-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &TravelokaRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for TravelokaConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "traveloka authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn subscribe_webhook(
        ...
        // Traveloka webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("traveloka-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<TravelokaWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<TravelokaWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<TravelokaWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("traveloka webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty traveloka webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.booking_id).ok_or_else(|| {
                ConnectorError::Webhook("traveloka webhook event missing booking_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "traveloka-key",
            "api_base_url": "https://api.test/traveloka",
            "webhook_secret": "traveloka-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        assert_eq!(token.access_token.expose(), "traveloka-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/traveloka/v1/bookings?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = TravelokaConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Traveloka-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/traveloka/v1/bookings?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Traveloka-Api-Key") && v == "traveloka-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/traveloka/v1/bookings?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/traveloka/v1/bookings?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/traveloka/v1/bookings/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Traveloka booking o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/traveloka")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "traveloka-secret");
    }
