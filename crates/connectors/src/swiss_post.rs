//! Swiss Post connector — Swiss Post partner API (`https://api.post.ch`).
//!
...
//! `X-SwissPost-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/parcels`
...
//! * `fetch_content` GETs a single parcel (`/v1/parcels/{id}`).
...
#[derive(Debug, Clone, Default, Deserialize)]
struct SwissPostWebhookEvent {
    #[serde(default)]
    parcel_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// Swiss Post connector.
pub struct SwissPostConnector {
...
impl std::fmt::Debug for SwissPostConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SwissPostConnector")
...
impl SwissPostConnector {
    /// Construct a Swiss Post connector.
    pub fn new(
    ...
    /// Override the Swiss Post API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("swiss_post", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "swiss_post {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<SwissPostRecord>> {
        let mut records = Vec::<SwissPostRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/parcels?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: SwissPostPage = self.http_get("/v1/parcels", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "swiss_post /v1/parcels exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-SwissPost-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-SwissPost-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &SwissPostRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for SwissPostConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "swiss_post authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/parcels/{id_enc}");
        let record: SwissPostRecord = self.http_get("/v1/parcels/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Swiss Post parcel {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Swiss Post parcel {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Swiss Post webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("swiss_post-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<SwissPostWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<SwissPostWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<SwissPostWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("swiss_post webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty swiss_post webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.parcel_id).ok_or_else(|| {
                ConnectorError::Webhook("swiss_post webhook event missing parcel_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "swiss_post-key",
            "api_base_url": "https://api.test/swiss_post",
            "webhook_secret": "swiss_post-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = SwissPostConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "swiss_post-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/swiss_post/v1/parcels?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = SwissPostConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-SwissPost-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/swiss_post/v1/parcels?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-SwissPost-Api-Key") && v == "swiss_post-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/swiss_post/v1/parcels?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/swiss_post/v1/parcels?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/swiss_post/v1/parcels/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample parcel"
        ...
        assert!(body.contains("# Swiss Post parcel o-1"));
        assert!(body.contains("Sample parcel"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/swiss_post",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "swiss_post-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let c = SwissPostConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let body =
            serde_json::to_vec(&serde_json::json!({ "parcel_id": 42, "event": "parcel.updated" }))
                .unwrap();
