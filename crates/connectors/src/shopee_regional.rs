//! Shopee Regional connector — Shopee Regional partner API (`https://partner.shopeemobile.com/api/v2`).
//!
...
//! `X-ShopeeRegional-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Shopee Regional API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://partner.shopeemobile.com/api/v2";
...
/// `X-ShopeeRegional-Api-Key` header) from an OAuth-issued bearer token.
...
struct ShopeeRegionalPage {
    #[serde(default)]
    data: Vec<ShopeeRegionalRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct ShopeeRegionalRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct ShopeeRegionalWebhookEvent {
    #[serde(default)]
...
/// Shopee Regional connector.
pub struct ShopeeRegionalConnector {
...
impl std::fmt::Debug for ShopeeRegionalConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ShopeeRegionalConnector")
...
impl ShopeeRegionalConnector {
    /// Construct a Shopee Regional connector.
    pub fn new(
    ...
    /// Override the Shopee Regional API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("shopee_regional", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "shopee_regional {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    ) -> Result<Vec<ShopeeRegionalRecord>> {
...
/// goes in the provider-native `X-ShopeeRegional-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-ShopeeRegional-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &ShopeeRegionalRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for ShopeeRegionalConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "shopee_regional authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Shopee Regional order {id}"))
            .with_metadata(serde_json::json!({
                "provider": "shopee_regional",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // Shopee Regional webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("shopee_regional-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<ShopeeRegionalWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<ShopeeRegionalWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<ShopeeRegionalWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("shopee_regional webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty shopee_regional webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.order_id).ok_or_else(|| {
                ConnectorError::Webhook("shopee_regional webhook event missing order_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "shopee_regional-key",
            "api_base_url": "https://api.test/shopee_regional",
            "webhook_secret": "shopee_regional-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = ShopeeRegionalConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "shopee_regional-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        let c =
            ShopeeRegionalConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-ShopeeRegional-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-ShopeeRegional-Api-Key")
                && v == "shopee_regional-key"));
    }
    ...
    fn fetch_content_renders_markdown() {
        ...
        assert!(body.contains("# Shopee Regional order o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/shopee_regional",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "shopee_regional-secret");
    }
