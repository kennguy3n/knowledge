//! Gojek connector — GoTo / Gojek partner API
//! (`https://api.gojek.com`).
//!
//! Gojek is the Indonesian super-app (part of the GoTo group). Its
//! partner API authenticates with a static API key presented in the
//! `X-Gojek-Api-Key` header, so [`GojekConnector::authenticate`]
//! reads the key out of `auth_config_json` and falls back to the
...
//! * Gojek webhooks are configured in the partner dashboard, so
...
/// Default Gojek partner API base URL.
...
/// Gojek partner connector.
pub struct GojekConnector {
...
impl std::fmt::Debug for GojekConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GojekConnector")
...
impl GojekConnector {
    ...
    /// Override the Gojek API base URL.
    #[must_use]
    ...
    fn gojek_get<R: DeserializeOwned>(
        &self,
        endpoint: &str,
        url: &str,
        token: &OAuth2Token,
        ...
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "gojek {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_orders(
    ...
    ) -> Result<Vec<GojekOrder>> {
        let mut orders = Vec::<GojekOrder>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: GojekOrdersPage = self.gojek_get("/v1/orders", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "gojek /v1/orders exceeded {MAX_PAGES} pages"
        )))
    }
}
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Gojek-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn order_watermark(o: &GojekOrder) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for GojekConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "gojek authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn initial_sync(&self, config: &ConnectorConfig, token: &OAuth2Token) -> Result<SyncRunResult> {
        ...
        for order in &orders {
            ...
            events.push(ConnectorEvent::DocumentCreated {
                document_id: SourceDocumentId::new(order.id.clone()),
                occurred_at,
            });
            if let Some(t) = order_watermark(order) {
                cursor.observe(t, &order.id);
            }
        }
        Ok(SyncRunResult {
            events,
    ...
    fn incremental_sync(
        ...
        for order in &orders {
            ...
            if !prior.should_emit(updated, &order.id) {
                continue;
            }
            ...
            cursor.observe(updated, &order.id);
        }
        Ok(SyncRunResult {
            events,
            next_cursor: cursor.to_cursor_string(),
    ...
    fn fetch_content(
        ...
        let order: GojekOrder = self.gojek_get("/v1/orders/{id}", &url, token)?;
        let status = order.status.as_deref().unwrap_or("unknown");
        let customer = order
            .customer_name
            .as_deref()
            .unwrap_or("(unknown customer)");
        let body = format!("# Gojek order {id}\n\nCustomer: {customer}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Gojek order {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Gojek webhooks are registered in the partner dashboard; no
        // REST endpoint creates them. Record a polling-only
        // subscription so the runtime falls back to incremental_sync.
        let secret = config
            .auth_config_json
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("gojek-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<GojekWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<GojekWebhookEvent>>(body)
        {
            batch
        } else {
            vec![serde_json::from_slice::<GojekWebhookEvent>(body)
                .map_err(|e| ConnectorError::Webhook(format!("gojek webhook parse failed: {e}")))?]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty gojek webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.order_id).ok_or_else(|| {
                ConnectorError::Webhook("gojek webhook event missing order_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Gojek, AuthKind::ApiKey, ScopeId::new_v4())
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = GojekConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::Gojek, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/gojek/v1/orders?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("x-gojek-api-key") && v == "gojek-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/gojek/v1/orders?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/gojek/v1/orders?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/gojek/v1/orders/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "customer_name": "Budi"
        ...
        assert!(body.contains("# Gojek order o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/gojek")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "gojek-secret");
    }
