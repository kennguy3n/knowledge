//! Deutsche Post connector — Deutsche Post partner API (`https://api.deutschepost.de`).
//!
...
//! `X-DeutschePost-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Deutsche Post API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.deutschepost.de";
...
/// `X-DeutschePost-Api-Key` header) from an OAuth-issued bearer token.
...
struct DeutschePostPage {
    #[serde(default)]
    data: Vec<DeutschePostRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct DeutschePostRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct DeutschePostWebhookEvent {
    #[serde(default)]
...
/// Deutsche Post connector.
pub struct DeutschePostConnector {
...
impl std::fmt::Debug for DeutschePostConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DeutschePostConnector")
...
impl DeutschePostConnector {
    /// Construct a Deutsche Post connector.
    pub fn new(
    ...
    /// Override the Deutsche Post API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("deutsche_post", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "deutsche_post {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<DeutschePostRecord>> {
        let mut records = Vec::<DeutschePostRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: DeutschePostPage = self.http_get("/v1/shipments", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "deutsche_post /v1/shipments exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-DeutschePost-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-DeutschePost-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &DeutschePostRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for DeutschePostConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "deutsche_post authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: DeutschePostRecord = self.http_get("/v1/shipments/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Deutsche Post shipment {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Deutsche Post shipment {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Deutsche Post webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("deutsche_post-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<DeutschePostWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<DeutschePostWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<DeutschePostWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("deutsche_post webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty deutsche_post webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.shipment_id).ok_or_else(|| {
                ConnectorError::Webhook("deutsche_post webhook event missing shipment_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "deutsche_post-key",
            "api_base_url": "https://api.test/deutsche_post",
            "webhook_secret": "deutsche_post-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = DeutschePostConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "deutsche_post-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deutsche_post/v1/shipments?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            DeutschePostConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-DeutschePost-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deutsche_post/v1/shipments?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-DeutschePost-Api-Key")
                && v == "deutsche_post-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/deutsche_post/v1/shipments?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/deutsche_post/v1/shipments?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/deutsche_post/v1/shipments/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Deutsche Post shipment o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/deutsche_post",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "deutsche_post-secret");
    }
