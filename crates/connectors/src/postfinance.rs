//! PostFinance connector — PostFinance partner API (`https://api.postfinance.ch`).
//!
...
//! `X-PostFinance-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default PostFinance API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.postfinance.ch";
...
/// `X-PostFinance-Api-Key` header) from an OAuth-issued bearer token.
...
struct PostFinancePage {
    #[serde(default)]
    data: Vec<PostFinanceRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct PostFinanceRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct PostFinanceWebhookEvent {
    #[serde(default)]
...
/// PostFinance connector.
pub struct PostFinanceConnector {
...
impl std::fmt::Debug for PostFinanceConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PostFinanceConnector")
...
impl PostFinanceConnector {
    /// Construct a PostFinance connector.
    pub fn new(
    ...
    /// Override the PostFinance API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("postfinance", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "postfinance {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<PostFinanceRecord>> {
        let mut records = Vec::<PostFinanceRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: PostFinancePage = self.http_get("/v1/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "postfinance /v1/transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-PostFinance-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-PostFinance-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &PostFinanceRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for PostFinanceConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "postfinance authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: PostFinanceRecord = self.http_get("/v1/transactions/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# PostFinance transaction {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("PostFinance transaction {id}"))
    ...
    fn subscribe_webhook(
        ...
        // PostFinance webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("postfinance-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<PostFinanceWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<PostFinanceWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<PostFinanceWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("postfinance webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty postfinance webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.transaction_id).ok_or_else(|| {
                ConnectorError::Webhook("postfinance webhook event missing transaction_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "postfinance-key",
            "api_base_url": "https://api.test/postfinance",
            "webhook_secret": "postfinance-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = PostFinanceConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "postfinance-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/postfinance/v1/transactions?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            PostFinanceConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-PostFinance-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/postfinance/v1/transactions?limit=2&offset=2",
        ...
        assert!(recorded[0].headers.iter().any(|(k, v)| k
            .eq_ignore_ascii_case("X-PostFinance-Api-Key")
            && v == "postfinance-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/postfinance/v1/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/postfinance/v1/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/postfinance/v1/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# PostFinance transaction o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/postfinance",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "postfinance-secret");
    }
