//! AirAsia Super App connector — AirAsia Super App partner API (`https://api.airasia.com`).
//!
...
//! `X-AirAsiaSuperApp-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default AirAsia Super App API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.airasia.com";
...
/// `X-AirAsiaSuperApp-Api-Key` header) from an OAuth-issued bearer token.
...
struct AirAsiaSuperAppPage {
    #[serde(default)]
    data: Vec<AirAsiaSuperAppRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct AirAsiaSuperAppRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct AirAsiaSuperAppWebhookEvent {
    #[serde(default)]
...
/// AirAsia Super App connector.
pub struct AirAsiaSuperAppConnector {
...
impl std::fmt::Debug for AirAsiaSuperAppConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AirAsiaSuperAppConnector")
...
impl AirAsiaSuperAppConnector {
    /// Construct a AirAsia Super App connector.
    pub fn new(
    ...
    /// Override the AirAsia Super App API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("airasia_superapp", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "airasia_superapp {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<AirAsiaSuperAppRecord>> {
        let mut records = Vec::<AirAsiaSuperAppRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: AirAsiaSuperAppPage = self.http_get("/v1/orders", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "airasia_superapp /v1/orders exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-AirAsiaSuperApp-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-AirAsiaSuperApp-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &AirAsiaSuperAppRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for AirAsiaSuperAppConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "airasia_superapp authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: AirAsiaSuperAppRecord = self.http_get("/v1/orders/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# AirAsia Super App order {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("AirAsia Super App order {id}"))
            .with_metadata(serde_json::json!({
                "provider": "airasia_superapp",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // AirAsia Super App webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("airasia_superapp-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<AirAsiaSuperAppWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<AirAsiaSuperAppWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<AirAsiaSuperAppWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("airasia_superapp webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty airasia_superapp webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.order_id).ok_or_else(|| {
                ConnectorError::Webhook("airasia_superapp webhook event missing order_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "airasia_superapp-key",
            "api_base_url": "https://api.test/airasia_superapp",
            "webhook_secret": "airasia_superapp-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = AirAsiaSuperAppConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "airasia_superapp-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/airasia_superapp/v1/orders?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = AirAsiaSuperAppConnector::new(
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-AirAsiaSuperApp-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/airasia_superapp/v1/orders?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-AirAsiaSuperApp-Api-Key")
                && v == "airasia_superapp-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/airasia_superapp/v1/orders?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/airasia_superapp/v1/orders?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/airasia_superapp/v1/orders/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample order"
        ...
        assert!(body.contains("# AirAsia Super App order o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/airasia_superapp",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "airasia_superapp-secret");
    }
