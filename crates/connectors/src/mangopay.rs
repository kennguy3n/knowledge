...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/payins`
...
//! * `fetch_content` GETs a single payin (`/v1/payins/{id}`).
...
impl MangoPayConnector {
    ...
    fn paginate_records(
        ...
        for page in 0..MAX_PAGES {
            ...
            let mut url = format!(
                "{base_url}/v1/payins?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: MangoPayPage = self.http_get("/v1/payins", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "mangopay /v1/payins exceeded {MAX_PAGES} pages"
        )))
    }
}
...
impl Connector for MangoPayConnector {
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/payins/{id_enc}");
        let record: MangoPayRecord = self.http_get("/v1/payins/{id}", &url, token)?;
...
mod tests {
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/mangopay/v1/payins?limit=2&offset=0",
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/mangopay/v1/payins?limit=2&offset=2",
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/mangopay/v1/payins?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/mangopay/v1/payins?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/mangopay/v1/payins/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample payin"
