//! Qonto connector — Qonto partner API (`https://thirdparty.qonto.com/v2`).
//!
...
/// Default Qonto API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://thirdparty.qonto.com/v2";
...
impl std::fmt::Debug for QontoConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("QontoConnector")
...
impl QontoConnector {
    ...
    /// Override the Qonto API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("qonto", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "qonto {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
        ...
        for page in 0..MAX_PAGES {
            ...
            let resp: QontoPage = self.http_get("/v1/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "qonto /v1/transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
fn record_watermark(o: &QontoRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for QontoConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "qonto authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: QontoRecord = self.http_get("/v1/transactions/{id}", &url, token)?;
    ...
    fn subscribe_webhook(
        ...
        // Qonto webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("qonto-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<QontoWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<QontoWebhookEvent>>(body)
        {
            batch
        } else {
            vec![serde_json::from_slice::<QontoWebhookEvent>(body)
                .map_err(|e| ConnectorError::Webhook(format!("qonto webhook parse failed: {e}")))?]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty qonto webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.transaction_id).ok_or_else(|| {
                ConnectorError::Webhook("qonto webhook event missing transaction_id".into())
            })?;
...
mod tests {
    ...
    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Qonto, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/qonto/v1/transactions?limit=2&offset=0",
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = QontoConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::Qonto, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/qonto/v1/transactions?limit=2&offset=2",
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/qonto/v1/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/qonto/v1/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/qonto/v1/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Qonto transaction o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/qonto")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "qonto-secret");
    }
