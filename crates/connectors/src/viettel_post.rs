...
impl Connector for ViettelPostConnector {
    ...
    fn initial_sync(&self, config: &ConnectorConfig, token: &OAuth2Token) -> Result<SyncRunResult> {
        ...
        for o in &orders {
            ...
            if let Some(ts) = o.updated_at {
                cursor.observe(ts, &o.order_number);
            }
        }
        Ok(SyncRunResult {
            events,
    ...
    fn incremental_sync(
        ...
        for o in &orders {
            match o.updated_at {
                Some(ts) => {
                    if !prior.should_emit(ts, &o.order_number) {
                        continue;
                    }
    ...
    fn incremental_sync_dedups_boundary_but_surfaces_new_same_second() {
        ...
        transport.expect(
            ...
            ok_json(&serde_json::json!({ "data": [
                ...
                order("VTP3", "2024-01-01T00:00:00Z"),
        ...
        state.cursor = Some("2024-01-01T00:00:00+00:00|VTP1".to_string());
        ...
        let ids: Vec<&str> = res
            ...
            .collect();
        assert_eq!(ids, vec!["VTP3", "VTP2"]);
        assert_eq!(
            res.next_cursor.as_deref(),
            Some("2024-04-01T00:00:00+00:00|VTP2")
        );
    }
