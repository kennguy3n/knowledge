//! Revolut Business connector — Revolut Business partner API (`https://b2b.revolut.com/api/1.0`).
//!
...
//! `X-RevolutBusiness-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Revolut Business API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://b2b.revolut.com/api/1.0";
...
/// `X-RevolutBusiness-Api-Key` header) from an OAuth-issued bearer token.
...
struct RevolutBusinessPage {
    #[serde(default)]
    data: Vec<RevolutBusinessRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct RevolutBusinessRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct RevolutBusinessWebhookEvent {
    #[serde(default)]
...
pub struct RevolutBusinessConnector {
...
impl std::fmt::Debug for RevolutBusinessConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RevolutBusinessConnector")
...
impl RevolutBusinessConnector {
    /// Construct a Revolut Business connector.
    pub fn new(
    ...
    /// Override the Revolut Business API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("revolut_business", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "revolut_business {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<RevolutBusinessRecord>> {
        let mut records = Vec::<RevolutBusinessRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: RevolutBusinessPage = self.http_get("/v1/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "revolut_business /v1/transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-RevolutBusiness-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-RevolutBusiness-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &RevolutBusinessRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for RevolutBusinessConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "revolut_business authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: RevolutBusinessRecord = self.http_get("/v1/transactions/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body =
            format!("# Revolut Business transaction {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Revolut Business transaction {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Revolut Business webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("revolut_business-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<RevolutBusinessWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<RevolutBusinessWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<RevolutBusinessWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("revolut_business webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty revolut_business webhook batch".into(),
            ));
        }
        ...
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.transaction_id).ok_or_else(|| {
                ConnectorError::Webhook(
                    "revolut_business webhook event missing transaction_id".into(),
                )
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "revolut_business-key",
            "api_base_url": "https://api.test/revolut_business",
            "webhook_secret": "revolut_business-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = RevolutBusinessConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "revolut_business-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/revolut_business/v1/transactions?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = RevolutBusinessConnector::new(
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-RevolutBusiness-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/revolut_business/v1/transactions?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-RevolutBusiness-Api-Key")
                && v == "revolut_business-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/revolut_business/v1/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/revolut_business/v1/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/revolut_business/v1/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# Revolut Business transaction o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/revolut_business",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "revolut_business-secret");
    }
