//! Companies House connector — Companies House partner API (`https://api.company-information.service.gov.uk`).
//!
...
//! `X-CompaniesHouse-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/filings`
...
//! * `fetch_content` GETs a single filing (`/v1/filings/{id}`).
...
#[derive(Debug, Clone, Default, Deserialize)]
struct CompaniesHouseWebhookEvent {
    #[serde(default)]
    filing_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// Companies House connector.
pub struct CompaniesHouseConnector {
...
impl std::fmt::Debug for CompaniesHouseConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CompaniesHouseConnector")
...
impl CompaniesHouseConnector {
    /// Construct a Companies House connector.
    pub fn new(
    ...
    /// Override the Companies House API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("companies_house", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "companies_house {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<CompaniesHouseRecord>> {
        let mut records = Vec::<CompaniesHouseRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/filings?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: CompaniesHousePage = self.http_get("/v1/filings", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "companies_house /v1/filings exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-CompaniesHouse-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-CompaniesHouse-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &CompaniesHouseRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for CompaniesHouseConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "companies_house authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/filings/{id_enc}");
        let record: CompaniesHouseRecord = self.http_get("/v1/filings/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# Companies House filing {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("Companies House filing {id}"))
    ...
    fn subscribe_webhook(
        ...
        // Companies House webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("companies_house-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<CompaniesHouseWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<CompaniesHouseWebhookEvent>>(body)
        {
            batch
        } else {
            vec![
                serde_json::from_slice::<CompaniesHouseWebhookEvent>(body).map_err(|e| {
                    ConnectorError::Webhook(format!("companies_house webhook parse failed: {e}"))
                })?,
            ]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty companies_house webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.filing_id).ok_or_else(|| {
                ConnectorError::Webhook("companies_house webhook event missing filing_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "companies_house-key",
            "api_base_url": "https://api.test/companies_house",
            "webhook_secret": "companies_house-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = CompaniesHouseConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "companies_house-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/companies_house/v1/filings?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c =
            CompaniesHouseConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
                .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-CompaniesHouse-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/companies_house/v1/filings?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-CompaniesHouse-Api-Key")
                && v == "companies_house-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/companies_house/v1/filings?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/companies_house/v1/filings?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/companies_house/v1/filings/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample filing"
        ...
        assert!(body.contains("# Companies House filing o-1"));
        assert!(body.contains("Sample filing"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/companies_house",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "companies_house-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let c = CompaniesHouseConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let body =
            serde_json::to_vec(&serde_json::json!({ "filing_id": 42, "event": "filing.updated" }))
                .unwrap();
