//! GoCardless connector — GoCardless partner API (`https://api.gocardless.com`).
//!
...
//! `X-GoCardless-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default GoCardless API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.gocardless.com";
...
/// `X-GoCardless-Api-Key` header) from an OAuth-issued bearer token.
...
struct GoCardlessPage {
    #[serde(default)]
    data: Vec<GoCardlessRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct GoCardlessRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl std::fmt::Debug for GoCardlessConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GoCardlessConnector")
...
impl GoCardlessConnector {
    /// Construct a GoCardless connector.
    pub fn new(
    ...
    /// Override the GoCardless API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("go_cardless", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "go_cardless {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<GoCardlessRecord>> {
        let mut records = Vec::<GoCardlessRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: GoCardlessPage = self.http_get("/v1/payments", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "go_cardless /v1/payments exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-GoCardless-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-GoCardless-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &GoCardlessRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for GoCardlessConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "go_cardless authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: GoCardlessRecord = self.http_get("/v1/payments/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# GoCardless payment {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("GoCardless payment {id}"))
    ...
    fn subscribe_webhook(
        ...
        // GoCardless webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("go_cardless-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<GoCardlessWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<GoCardlessWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<GoCardlessWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("go_cardless webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty go_cardless webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.payment_id).ok_or_else(|| {
                ConnectorError::Webhook("go_cardless webhook event missing payment_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "go_cardless-key",
            "api_base_url": "https://api.test/go_cardless",
            "webhook_secret": "go_cardless-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = GoCardlessConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "go_cardless-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/go_cardless/v1/payments?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = GoCardlessConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-GoCardless-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/go_cardless/v1/payments?limit=2&offset=2",
        ...
        assert!(
            recorded[0]
                .headers
                .iter()
                .any(|(k, v)| k.eq_ignore_ascii_case("X-GoCardless-Api-Key")
                    && v == "go_cardless-key")
        );
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/go_cardless/v1/payments?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/go_cardless/v1/payments?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/go_cardless/v1/payments/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# GoCardless payment o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/go_cardless",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "go_cardless-secret");
    }
