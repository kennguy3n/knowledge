//! SEEK connector — SEEK partner API (`https://api.seek.com`).
//!
...
//! `X-Seek-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/ads`
...
//! * `fetch_content` GETs a single ad (`/v1/ads/{id}`).
...
#[derive(Debug, Clone, Default, Deserialize)]
struct SeekWebhookEvent {
    #[serde(default)]
    ad_id: serde_json::Value,
    #[serde(default)]
    event: String,
}
...
impl std::fmt::Debug for SeekConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SeekConnector")
...
impl SeekConnector {
    ...
    /// Override the SEEK API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("seek", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "seek {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<SeekRecord>> {
        let mut records = Vec::<SeekRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!("{base_url}/v1/ads?limit={}&offset={offset}", self.page_size);
            if let Some(since) = modified_since {
                url.push_str("&modified_since=");
                url.push_str(&percent_encode_path_component(since));
            }
            let resp: SeekPage = self.http_get("/v1/ads", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "seek /v1/ads exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Seek-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Seek-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &SeekRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for SeekConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "seek authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/ads/{id_enc}");
        let record: SeekRecord = self.http_get("/v1/ads/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# SEEK ad {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("SEEK ad {id}"))
    ...
    fn subscribe_webhook(
        ...
        // SEEK webhooks are registered in the provider
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<SeekWebhookEvent> = if let Ok(batch) =
            serde_json::from_slice::<Vec<SeekWebhookEvent>>(body)
        {
            batch
        } else {
            vec![serde_json::from_slice::<SeekWebhookEvent>(body)
                .map_err(|e| ConnectorError::Webhook(format!("seek webhook parse failed: {e}")))?]
        };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook("empty seek webhook batch".into()));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.ad_id).ok_or_else(|| {
                ConnectorError::Webhook("seek webhook event missing ad_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Seek, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "seek-key",
                "api_base_url": "https://api.test/seek",
                "webhook_secret": "seek-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::Seek, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = SeekConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "seek-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/seek/v1/ads?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = SeekConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Seek-Api-Key")));
    }
    ...
    fn authenticate_requires_credential() {
        let transport = Arc::new(MockHttpTransport::new());
        let c = SeekConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let bare = ConnectorConfig::new(ConnectorKind::Seek, AuthKind::ApiKey, ScopeId::new_v4());
        assert!(matches!(
            c.authenticate(&bare),
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/seek/v1/ads?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Seek-Api-Key") && v == "seek-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/seek/v1/ads?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/seek/v1/ads?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
            ok_json(&serde_json::json!({ "data": [] })),
        );
        let c =
            SeekConnector::new(ConnectorInstanceId::new_v4(), transport, oauth()).with_page_size(2);
        let tok = c.authenticate(&cfg()).unwrap();
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/seek/v1/ads/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample ad"
        ...
        assert!(body.contains("# SEEK ad o-1"));
        assert!(body.contains("Sample ad"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/seek")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "seek-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let c = SeekConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let body =
            serde_json::to_vec(&serde_json::json!({ "ad_id": 42, "event": "ad.updated" })).unwrap();
