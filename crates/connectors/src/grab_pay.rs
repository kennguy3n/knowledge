//! GrabPay connector — GrabPay partner API (`https://partner-api.grab.com/grabpay`).
//!
...
//! `X-GrabPay-Api-Key` header (read from `auth_config_json.api_key`),
...
//!
//! * `initial_sync` / `incremental_sync` page `/v1/charges`
...
//! * `fetch_content` GETs a single charge (`/v1/charges/{id}`).
...
/// Default GrabPay API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://partner-api.grab.com/grabpay";
/// Default scope recorded on the synthesised API-key token.
pub const DEFAULT_SCOPE: &str = "charges";
/// `OAuth2Token::token_type` marker for a static API-key credential.
/// Distinguishes the API-key auth path (provider-native
/// `X-GrabPay-Api-Key` header) from an OAuth-issued bearer token.
pub const API_KEY_TOKEN_TYPE: &str = "ApiKey";
/// Page size for charge listing (`limit`).
...
struct GrabPayPage {
    #[serde(default)]
    data: Vec<GrabPayRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct GrabPayRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Default, Deserialize)]
struct GrabPayWebhookEvent {
    #[serde(default)]
    charge_id: serde_json::Value,
    #[serde(default)]
    event: String,
}

/// GrabPay connector.
pub struct GrabPayConnector {
...
impl std::fmt::Debug for GrabPayConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GrabPayConnector")
...
impl GrabPayConnector {
    /// Construct a GrabPay connector.
    pub fn new(
    ...
    /// Override the GrabPay API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("grab_pay", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "grab_pay {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<GrabPayRecord>> {
        let mut records = Vec::<GrabPayRecord>::new();
        for page in 0..MAX_PAGES {
            let offset = page * self.page_size as usize;
            let mut url = format!(
                "{base_url}/v1/charges?limit={}&offset={offset}",
                self.page_size
            );
            ...
            let resp: GrabPayPage = self.http_get("/v1/charges", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "grab_pay /v1/charges exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-GrabPay-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-GrabPay-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &GrabPayRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for GrabPayConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "grab_pay authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let url = format!("{base_url}/v1/charges/{id_enc}");
        let record: GrabPayRecord = self.http_get("/v1/charges/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# GrabPay charge {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("GrabPay charge {id}"))
    ...
    fn subscribe_webhook(
        ...
        // GrabPay webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("grab_pay-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<GrabPayWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<GrabPayWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<GrabPayWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("grab_pay webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty grab_pay webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.charge_id).ok_or_else(|| {
                ConnectorError::Webhook("grab_pay webhook event missing charge_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::GrabPay, AuthKind::ApiKey, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
                "api_key": "grab_pay-key",
                "api_base_url": "https://api.test/grab_pay",
                "webhook_secret": "grab_pay-secret",
            }))
    }

    fn cfg_oauth() -> ConnectorConfig {
        ConnectorConfig::new(ConnectorKind::GrabPay, AuthKind::OAuth2, ScopeId::new_v4())
            .with_auth_config(serde_json::json!({
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = GrabPayConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "grab_pay-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/grab_pay/v1/charges?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = GrabPayConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-GrabPay-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/grab_pay/v1/charges?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-GrabPay-Api-Key") && v == "grab_pay-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/grab_pay/v1/charges?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/grab_pay/v1/charges?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/grab_pay/v1/charges/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample charge"
        ...
        assert!(body.contains("# GrabPay charge o-1"));
        assert!(body.contains("Sample charge"));
    }
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/grab_pay")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "grab_pay-secret");
    }
    ...
    fn handle_webhook_event_parses_single() {
        ...
        let c = GrabPayConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let body =
            serde_json::to_vec(&serde_json::json!({ "charge_id": 42, "event": "charge.updated" }))
                .unwrap();
    ...
    fn handle_webhook_event_maps_event_kinds() {
        ...
        let body = serde_json::to_vec(&serde_json::json!([
            { "charge_id": "x-1", "event": "charge.created" },
            { "charge_id": "x-2", "event": "charge.cancelled" },
            { "charge_id": "x-3", "event": "charge.deleted" },
            { "charge_id": "x-4", "event": "charge.updated" }
        ]))
        .unwrap();
        let events = c.handle_webhook_event(&body).unwrap();
