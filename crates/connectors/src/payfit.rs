//! PayFit connector — PayFit partner API (`https://api.payfit.com`).
//!
...
//! `X-PayFit-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default PayFit API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.payfit.com";
...
/// `X-PayFit-Api-Key` header) from an OAuth-issued bearer token.
...
impl PayFitConnector {
    ...
    fn paginate_records(
        ...
        for page in 0..MAX_PAGES {
            ...
            let resp: PayFitPage = self.http_get("/v1/employees", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "payfit /v1/employees exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-PayFit-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-PayFit-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &PayFitRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for PayFitConnector {
    ...
    fn subscribe_webhook(
        ...
        // PayFit webhooks are registered in the provider
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        ...
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.employee_id).ok_or_else(|| {
                ConnectorError::Webhook("payfit webhook event missing employee_id".into())
            })?;
...
mod tests {
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/payfit/v1/employees?limit=2&offset=0",
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/payfit/v1/employees?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-PayFit-Api-Key") && v == "payfit-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/payfit/v1/employees?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/payfit/v1/employees?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/payfit/v1/employees/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# PayFit employee o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/payfit")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "payfit-secret");
    }
