...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "tiki-api-key", API_KEY_TOKEN_TYPE)
}
...
impl Connector for TikiConnector {
    ...
    fn incremental_sync(
        ...
        let orders =
            self.paginate_orders(&base_url, &secret, token, prior.query_since().as_deref())?;
        let mut events = Vec::new();
        let mut cursor = prior.clone();
        for o in &orders {
            match o.updated_at {
                Some(ts) => {
                    if !prior.should_emit(ts, &o.code) {
                        continue;
                    }
...
mod tests {
    ...
    fn incremental_sync_dedups_boundary_but_surfaces_new_same_second() {
        ...
        transport.with_default_response(ok_json(&serde_json::json!({ "data": [
            ...
            order("OD3", "2024-01-01T00:00:00Z"),
            order("OD2", "2024-05-01T00:00:00Z"),
        ] })));
        let c = TikiConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth());
        let tok = c.authenticate(&cfg()).unwrap();
        let mut state = SyncState::new(c.instance);
        state.cursor = Some("2024-01-01T00:00:00+00:00|OD1".to_string());
        ...
        let ids: Vec<&str> = res
            ...
            .collect();
        assert_eq!(ids, vec!["OD3", "OD2"]);
        assert_eq!(
            res.next_cursor.as_deref(),
            Some("2024-05-01T00:00:00+00:00|OD2")
        );
