//! lexoffice connector — lexoffice partner API (`https://api.lexoffice.io`).
//!
...
//! `X-Lexoffice-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default lexoffice API base URL (host-only; request paths add `/v1`).
pub const DEFAULT_API_BASE_URL: &str = "https://api.lexoffice.io";
...
/// `X-Lexoffice-Api-Key` header) from an OAuth-issued bearer token.
...
struct LexofficePage {
    #[serde(default)]
    data: Vec<LexofficeRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct LexofficeRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl std::fmt::Debug for LexofficeConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LexofficeConnector")
...
impl LexofficeConnector {
    /// Construct a lexoffice connector.
    pub fn new(
    ...
    /// Override the lexoffice API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("lexoffice", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "lexoffice {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    ) -> Result<Vec<LexofficeRecord>> {
        let mut records = Vec::<LexofficeRecord>::new();
...
/// goes in the provider-native `X-Lexoffice-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Lexoffice-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &LexofficeRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for LexofficeConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "lexoffice authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("lexoffice invoice {id}"))
    ...
    fn subscribe_webhook(
        ...
        // lexoffice webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("lexoffice-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<LexofficeWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<LexofficeWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<LexofficeWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("lexoffice webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty lexoffice webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.invoice_id).ok_or_else(|| {
                ConnectorError::Webhook("lexoffice webhook event missing invoice_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "lexoffice-key",
            "api_base_url": "https://api.test/lexoffice",
            "webhook_secret": "lexoffice-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = LexofficeConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "lexoffice-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-Lexoffice-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Lexoffice-Api-Key") && v == "lexoffice-key"));
    }
    ...
    fn fetch_content_renders_markdown() {
        ...
        assert!(body.contains("# lexoffice invoice o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/lexoffice")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "lexoffice-secret");
    }
    ...
    fn default_base_url_has_no_duplicate_version() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!("{DEFAULT_API_BASE_URL}/v1/invoices?limit=2&offset=0"),
        ...
        .with_auth_config(serde_json::json!({ "api_key": "lexoffice-key" }));
