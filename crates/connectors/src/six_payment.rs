//! SIX Payment connector — SIX Payment partner API (`https://api.six-payment-services.com`).
//!
...
//! `X-SixPayment-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default SIX Payment API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://api.six-payment-services.com";
...
/// `X-SixPayment-Api-Key` header) from an OAuth-issued bearer token.
...
struct SixPaymentPage {
    #[serde(default)]
    data: Vec<SixPaymentRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct SixPaymentRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
/// SIX Payment connector.
pub struct SixPaymentConnector {
...
impl std::fmt::Debug for SixPaymentConnector {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SixPaymentConnector")
...
impl SixPaymentConnector {
    /// Construct a SIX Payment connector.
    pub fn new(
    ...
    /// Override the SIX Payment API base URL.
    #[must_use]
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("six_payment", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "six_payment {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<SixPaymentRecord>> {
        let mut records = Vec::<SixPaymentRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: SixPaymentPage = self.http_get("/v1/transactions", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "six_payment /v1/transactions exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-SixPayment-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-SixPayment-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &SixPaymentRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for SixPaymentConnector {
    fn authenticate(&self, config: &ConnectorConfig) -> Result<OAuth2Token> {
        ...
        let auth_code = config
            ...
            .ok_or_else(|| {
                ConnectorError::Auth(
                    "six_payment authenticate: auth_config_json.api_key or .authorization_code is required"
                        .into(),
                )
            })?;
    ...
    fn fetch_content(
        ...
        let record: SixPaymentRecord = self.http_get("/v1/transactions/{id}", &url, token)?;
        let status = record.status.as_deref().unwrap_or("unknown");
        let title = record.title.as_deref().unwrap_or("(untitled)");
        let body = format!("# SIX Payment transaction {id}\n\nTitle: {title}\nStatus: {status}\n");
        Ok(FetchedContent::text(body, "text/markdown")
            .with_title(format!("SIX Payment transaction {id}"))
            .with_metadata(serde_json::json!({
                "provider": "six_payment",
                "record_id": record.id,
    ...
    fn subscribe_webhook(
        ...
        // SIX Payment webhooks are registered in the provider
        ...
        let secret = config
            ...
            .get("webhook_secret")
            .and_then(serde_json::Value::as_str)
            .unwrap_or("six_payment-webhook-secret");
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        let deliveries: Vec<SixPaymentWebhookEvent> =
            if let Ok(batch) = serde_json::from_slice::<Vec<SixPaymentWebhookEvent>>(body) {
                batch
            } else {
                vec![
                    serde_json::from_slice::<SixPaymentWebhookEvent>(body).map_err(|e| {
                        ConnectorError::Webhook(format!("six_payment webhook parse failed: {e}"))
                    })?,
                ]
            };
        if deliveries.is_empty() {
            return Err(ConnectorError::Webhook(
                "empty six_payment webhook batch".into(),
            ));
        }
        let mut events = Vec::with_capacity(deliveries.len());
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.transaction_id).ok_or_else(|| {
                ConnectorError::Webhook("six_payment webhook event missing transaction_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "six_payment-key",
            "api_base_url": "https://api.test/six_payment",
            "webhook_secret": "six_payment-secret",
        }))
    }
    ...
    fn authenticate_reads_api_key() {
        ...
        let c = SixPaymentConnector::new(ConnectorInstanceId::new_v4(), transport, oauth());
        let token = c.authenticate(&cfg()).unwrap();
        assert_eq!(token.access_token.expose(), "six_payment-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/six_payment/v1/transactions?limit=2&offset=0",
            ok_json(&serde_json::json!({
                "data": [ {"id": "o-1", "updated_at": "2024-01-01T00:00:00Z"} ]
            })),
        );
        let c = SixPaymentConnector::new(ConnectorInstanceId::new_v4(), transport.clone(), oauth())
            .with_page_size(2);
        ...
        assert!(!recorded[0]
            .headers
            .iter()
            .any(|(k, _)| k.eq_ignore_ascii_case("X-SixPayment-Api-Key")));
    }
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/six_payment/v1/transactions?limit=2&offset=2",
        ...
        assert!(
            recorded[0]
                .headers
                .iter()
                .any(|(k, v)| k.eq_ignore_ascii_case("X-SixPayment-Api-Key")
                    && v == "six_payment-key")
        );
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/six_payment/v1/transactions?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/six_payment/v1/transactions?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/six_payment/v1/transactions/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
        ...
        assert!(body.contains("# SIX Payment transaction o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let sub = c
            .subscribe_webhook(
                &cfg(),
                &tok,
                "https://substrate.example/webhooks/six_payment",
            )
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "six_payment-secret");
    }
