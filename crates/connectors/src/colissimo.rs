//! Colissimo connector — Colissimo partner API (`https://ws.colissimo.fr`).
//!
...
//! `X-Colissimo-Api-Key` header (read from `auth_config_json.api_key`),
...
/// Default Colissimo API base URL.
pub const DEFAULT_API_BASE_URL: &str = "https://ws.colissimo.fr";
...
/// `X-Colissimo-Api-Key` header) from an OAuth-issued bearer token.
...
struct ColissimoPage {
    #[serde(default)]
    data: Vec<ColissimoRecord>,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
struct ColissimoRecord {
    #[serde(default)]
    id: String,
    #[serde(default)]
...
impl ColissimoConnector {
    ...
    fn http_get<R: DeserializeOwned>(
        ...
        if !resp.is_success() {
            return Err(classify_failure("colissimo", endpoint, &resp));
        }
        serde_json::from_slice::<R>(&resp.body).map_err(|e| {
            ConnectorError::Sync(format!(
                "colissimo {endpoint} JSON parse failed: {e} (body prefix: {})",
    ...
    fn paginate_records(
    ...
    ) -> Result<Vec<ColissimoRecord>> {
        let mut records = Vec::<ColissimoRecord>::new();
        for page in 0..MAX_PAGES {
            ...
            let resp: ColissimoPage = self.http_get("/v1/parcels", &url, token)?;
        ...
        Err(ConnectorError::Sync(format!(
            "colissimo /v1/parcels exceeded {MAX_PAGES} pages"
        )))
    }
}
...
/// goes in the provider-native `X-Colissimo-Api-Key` header, while an
...
fn apply_auth(req: HttpRequest, token: &OAuth2Token) -> HttpRequest {
    apply_auth_by_provenance(req, token, "X-Colissimo-Api-Key", API_KEY_TOKEN_TYPE)
}
...
fn record_watermark(o: &ColissimoRecord) -> Option<DateTime<Utc>> {
    o.updated_at
        .as_deref()
...
impl Connector for ColissimoConnector {
    ...
    fn subscribe_webhook(
        ...
        // Colissimo webhooks are registered in the provider
    ...
    fn handle_webhook_event(&self, body: &[u8]) -> Result<Vec<ConnectorEvent>> {
        ...
        for delivery in deliveries {
            let id_str = id_value_to_string(&delivery.parcel_id).ok_or_else(|| {
                ConnectorError::Webhook("colissimo webhook event missing parcel_id".into())
            })?;
...
mod tests {
    ...
    fn cfg() -> ConnectorConfig {
        ...
        .with_auth_config(serde_json::json!({
            "api_key": "colissimo-key",
            "api_base_url": "https://api.test/colissimo",
    ...
    fn authenticate_reads_api_key() {
        ...
        assert_eq!(token.access_token.expose(), "colissimo-key");
    ...
    fn oauth_token_is_sent_as_bearer_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/colissimo/v1/parcels?limit=2&offset=0",
    ...
    fn initial_sync_paginates_and_sends_api_key_header() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/colissimo/v1/parcels?limit=2&offset=2",
        ...
        assert!(recorded[0]
            .headers
            .iter()
            .any(|(k, v)| k.eq_ignore_ascii_case("X-Colissimo-Api-Key") && v == "colissimo-key"));
    }
    ...
    fn incremental_sync_dedups_boundary() {
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/colissimo/v1/parcels?limit=2&offset=0&modified_since={}",
        ...
        transport.expect(
            HttpMethod::Get,
            format!(
                "https://api.test/colissimo/v1/parcels?limit=2&offset=2&modified_since={}",
                percent_encode_path_component(since)
            ),
    ...
    fn fetch_content_renders_markdown() {
        ...
        transport.expect(
            HttpMethod::Get,
            "https://api.test/colissimo/v1/parcels/o-1",
            ok_json(&serde_json::json!({
                "id": "o-1",
                "status": "COMPLETED",
                "title": "Sample parcel"
        ...
        assert!(body.contains("# Colissimo parcel o-1"));
    ...
    fn subscribe_webhook_is_polling_only() {
        ...
        let tok = c.authenticate(&cfg()).unwrap();
        let sub = c
            .subscribe_webhook(&cfg(), &tok, "https://substrate.example/webhooks/colissimo")
            .unwrap();
        assert!(sub.provider_subscription_id.is_none());
        assert_eq!(sub.secret.expose(), "colissimo-secret");
    }
