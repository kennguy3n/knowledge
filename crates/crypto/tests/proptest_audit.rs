...
use crypto::{EvidenceRef, ProvenanceAgent, ProvenanceBundle, ProvenanceSigner, SynthesisActivity};
use uuid::Uuid;
...
fn hybrid_secret_key_is_zeroed_on_drop() {
    ...
    // has a Drop impl (generated by `#[zeroize(drop)]`). If someone
    // removes `#[zeroize(drop)]` but keeps the `Zeroize` derive,
    // the `needs_drop` check catches it.
    fn assert_zeroize<T: zeroize::Zeroize>() {}
    assert_zeroize::<crypto::HybridSecretKey>();
    assert!(
        std::mem::needs_drop::<crypto::HybridSecretKey>(),
        "HybridSecretKey must implement Drop (via #[zeroize(drop)])"
    );
}
