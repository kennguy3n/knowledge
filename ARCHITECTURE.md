...
| **Connector Service** | `internal/connector` | 10 providers (Drive, OneDrive, Notion, Jira, Confluence, Figma, HubSpot, Slack, Email, Salesforce); OAuth2 token refresh; webhook subscription; incremental delta sync; real document-content fetching; persistent connector registrations (Postgres) |
| **Permission Service** | `internal/permission` | Zanzibar-style relation graph: grant/revoke/check tuples; SCIM v2 user/group provisioning joined to tuple store |
