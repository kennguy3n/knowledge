...
It evaluates two recorded corpora:
  ...
  * demos/multilingual-rollup/results/rollup_results.json — the per-language
        synthesis matrix (default on-device Qwen3.5-2B model), scored the same way.
...
def check_gate(personas: list[RecapScore], multilingual: list[RecapScore]) -> list[Failure]:
    ...
    def _check_group(group_name: str, scores: list[RecapScore], cfg: dict) -> None:
        ...
        for s in scores:
            ...
            # default model on CJK/Arabic) are baselined False and documented as
...
def build_report(personas: list[RecapScore], multilingual: list[RecapScore],
    ...
    a("")
    a("> **See also:** the per-recap scores below are rolled up **per language** "
      "(with an honest pending list) in the public "
      "[multilingual leaderboard](multilingual-leaderboard.md), regenerated by "
      "`demos/synthesis-eval/leaderboard.py`.")
    a("")

    # --- Multilingual matrix ------------------------------------------------ #
    a("## 1. Multilingual matrix — default on-device model (Qwen3.5-2B Q4_K_M)")
    a("")
    a("Same business situation expressed natively per language; the recap must "
      "come back **in-language**. This is where the default model is "
    ...
    a(f"_In-language: **{ml_inlang}/{len(multilingual)}** languages. "
      "The misses are the documented non-Latin limitation: the model "
      "drops to a placeholder or answers in English on the hardest scripts._")
    a("")

    # --- Personas ----------------------------------------------------------- #
    a("## 2. Executive personas — channel recap quality")
    a("")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_report(personas, multilingual, gate_ok)
